...
- With `providers.cacheKeepalive: economic`, the provider prompt cache is now kept warm across long idle gaps while work this session will resume from is still running, so a session that dispatches subagents and waits — or sits in a long foreground build — no longer re-pays the whole prefix as a cache write when it resumes; keeping it warm stops as soon as the work finishes or the spend stops being worth it. This now works on every provider whose request can be replayed and that reports cache activity, not just Anthropic — including Bedrock, the OpenAI, Google and Vertex families, and custom APIs.
- Prompt-cache outcomes are now recorded per request to `cache-observations.jsonl` beside `stats.db`, and the observed retention learned from them replaces the nominal 5-minute guess when scheduling the next keep-warm touch — including for the first keep-warm of a session, which now waits for the learned value instead of racing it. Hashes and token counts only; bounded reads with self-rotation, only for real on-disk sessions, and short conversations below the provider's cacheable-prefix floor are skipped so their uninformative silence cannot erode what real hits and misses measured.
- Added `providers.cacheKeepalive`: `legacy` (default) keeps the pre-existing Anthropic-only 3-touch prompt-cache keep-alive, `economic` opts into the gated lease that keeps replaying while a touch stays cheaper than rebuilding the entry — up to 24 touches, across every provider that supports it, and correspondingly more billed keep-alive requests.
