...
- Marketplace plugins installed through `~/.claude/plugins/` now contribute rules: the `claude-plugins` provider scans each plugin root's conventional `rules/*.md` and `rules/*.mdc` files and loads them through the standard rule pipeline, so plugin-shipped rules get the same frontmatter handling (`globs`, `alwaysApply`, conditions, scope) as every other provider. Previously only `omp-plugins` (extension packages) loaded rules, leaving marketplace rule packs silently inert while their skills, commands, hooks, tools, and MCP servers loaded normally. The provider registers at priority 70, so user `.omp/` and extension-package rules still shadow same-named plugin rules.
