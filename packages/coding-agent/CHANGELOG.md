...
- Fixed `vault://` resolving the same vault root to two different absolute-path spellings (short vs. long) on Windows hosts with an 8.3 short-name `TEMP`/profile: the sync bash path used `fs.realpathSync` (which keeps the 8.3 spelling) while the async read path used `fs.promises.realpath` (long form), so one `vault://` file had two string identities. The sync path now uses the long-form native realpath, so both resolve consistently ([#7911](https://github.com/can1357/oh-my-pi/issues/7911)).
