...
- Added a pre-dispatch read-only mismatch check in the task tool: when a chosen agent is read-only (or plan mode is active) and any task's assignment starts with a high-signal imperative write verb (`refactor`, `implement`, `edit`, `modify`, `rename`, `scaffold`, `migrate`, `install`, `fix`), the dispatch fails with a clear actionable error before the subagent boots — saving the system-prompt tokens the `[CAPABILITY]` block would otherwise still pay. Verb list is intentionally conservative; ambiguous descriptive verbs (`create`, `write`, `add`, `build`, `update`, `generate`, `remove`, etc.) are excluded so investigation prose like "Create a summary" or "Investigate how Foo generates JSON" still routes through ([#1013](https://github.com/can1357/oh-my-pi/issues/1013))
