...
- Added branch-history banner on session resume. When a resumed session's chronology has at least two branches, OMP renders `Branch history: <chain> (current)` near the top of the session (under the welcome on `--continue`/autoResume, inline with the "Resumed session" indicator when the session is picked from the selector) with per-segment dwell hints (e.g. `main (10m) · feat/x (current)`).
