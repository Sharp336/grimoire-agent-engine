...
- Added `interleaved` and `legacy_style` fields to the model configuration schema. These fields allow users to control reasoning content replay behavior: `interleaved: false` disables reasoning content replay for older models, and `legacy_style: true` forces the old OMP behavior (converting thinking to text) for providers that haven't been updated yet.
- Added `reasoning_details` to the `reasoningContentField` enum in the model configuration schema, allowing models to specify they use the array format for reasoning content.
