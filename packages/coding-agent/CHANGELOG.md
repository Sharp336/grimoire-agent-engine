...
### Fixed
- Fixed GitHub Copilot MCP discovery to load user-level `~/.copilot/mcp-config.json`, including both `servers` and `mcpServers` top-level layouts, so MCP servers configured for Copilot CLI are no longer silently ignored.
