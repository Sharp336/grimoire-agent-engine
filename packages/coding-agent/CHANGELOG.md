...
- Added `ctx.ui.setStatusSegment(...)` so extensions can contribute sanitized inline text to the status line in the editor's top border. Supports `options.side` (`left`/`right`) and `options.order` for deterministic placement among extension segments.
