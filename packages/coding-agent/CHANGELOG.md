...
- Added `providers.cacheKeepalive`: `legacy` (default) keeps the fixed 3-touch prompt-cache keep-alive, `economic` opts into the gated lease that keeps replaying while a touch stays cheaper than rebuilding the entry — up to 24 touches, and correspondingly more billed keep-alive requests.
