...
- Added opt-in Kitty subagent panes for npm/Bun installs: up to four current-generation direct `task` children can open child-bound transcript/prompt views while Agent Hub remains the full fallback; the shipped custom authorization policy, one-shot handoff, and setup guide keep the integration local, bounded, and reversible.
