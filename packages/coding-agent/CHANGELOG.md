...
- Added a "Code Search" setting (`mcp.zread.enabled`, off by default) under Providers that enables the built-in [Zread](https://zread.ai) MCP server for code reading, documentation search, and repository structure tools on open-source GitHub repositories. Requires Z.AI login (`ZAI_API_KEY`). Skipped if the user already has a manually-configured `zread` MCP server or has disabled it via the MCP denylist.
