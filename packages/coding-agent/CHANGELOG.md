...
- Added a `credential_disabled` extension event so extensions can subscribe via `pi.on("credential_disabled", handler)` and react when `AuthStorage` automatically soft-disables a credential (e.g. OAuth `invalid_grant`). Replaces the current `agent_end` errorMessage regex pattern downstream extensions have to match against. Handler payload is `{ type, provider, disabledCause }`. The session installs a startup-buffer wrapper at the top of `createAgentSession()` to capture events fired during model-restore and fallback-model probes (which run before the extension runner exists), then swaps in a bridge that fans events to the runner and composes with any handler the embedder already attached to `AuthStorage` (so a caller-provided `onCredentialDisabled` keeps firing alongside extension subscribers). On `session.dispose()` the bridge is marked disposed and the previous handler chain is walked past any disposed sibling bridges before restoration, so concurrent sub-sessions sharing one `AuthStorage` don't restore each other's dead bridges. The outer `createAgentSession()` catch also releases the wrapper and the bridge if startup throws before `session.dispose()` is wired, so repeated retries don't accumulate stale frames in the previous-handler chain. The single-handler `AuthStorage.onCredentialDisabled` model is the underlying source of this complexity; a follow-up refactor to a multi-subscriber `on/off` API is planned and will collapse the bridge dance to a single `subscribe`/`unsubscribe` per session.
