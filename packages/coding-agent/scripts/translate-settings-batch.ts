...
const MAP: Record<string, string> = {
	...
	FLAGS: "选项",
	EXAMPLES: "示例",
	"Show what the read tool will return for a path, URL, or internal URI": "显示 read 工具对路径、URL 或内部 URI 返回的内容",
...
const translationFiles = fs.readdirSync(LAN_DIR).filter(f => f.startsWith("zh-") && f.endsWith(".json") && !f.startsWith("_"));
