...
export interface ProviderCallGatewayRuntime {
	gateway: ProviderCallGateway;
	expectedDynamics: ProviderCallExpectedDynamicsByConfig;
}

const PROVIDER_CALL_GATEWAY_RUNTIME_ENV = [
...
/**
 * Builds the credential-free TBPCW002/TBPCR003 client from one reviewed
 * same-Pod socket and the immutable route dynamics used for fail-closed validation.
 */
export function createProviderCallGatewayRuntimeFromEnv(
	env: Record<string, string | undefined> = process.env,
): ProviderCallGatewayRuntime | undefined {
	const configured = PROVIDER_CALL_GATEWAY_RUNTIME_ENV.map(name => env[name]?.trim() ?? "");
	if (configured.every(value => value === "")) return undefined;
	if (configured.some(value => value === "")) {
		throw new Error("Incomplete provider-call gateway configuration");
	}
	const [socketPath, expectedDynamicsJson] = configured;
	if (!path.isAbsolute(socketPath) || path.normalize(socketPath) !== socketPath) {
		throw new Error("Provider-call gateway requires a canonical absolute Unix socket path");
	}
	const expectedDynamics = parseExpectedProviderCallDynamics(expectedDynamicsJson);
	if (Object.keys(expectedDynamics).length === 0) {
		throw new Error("Provider-call gateway immutable binding configuration is empty");
	}
	return {
		gateway: new UnixProviderCallGateway({ socketPath }),
		expectedDynamics,
	};
}
...
export function indexModelsByRequestId(
	...
	providerCallGatewayEnabled = false,
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	let modelById = indexModelsByRequestId(registry.getAll(), providersWithCreds, providerCallRuntime !== undefined);
	...
	const handle = startAuthGateway({
		...
		providerCallGateway: providerCallRuntime?.gateway,
		expectedProviderCallDynamics: providerCallRuntime?.expectedDynamics,
	});
