...
export class EngineRuntime {
	...
	async #openBinding(
		...
		let resolved: Awaited<ReturnType<NonNullable<EngineRuntimeOptions["resolveSessionProfile"]>>> | undefined;
		...
		try {
			if (preparedSessionManager) {
				...
				if (!uncommittedForkSessionFile) throw new Error("Prepared session was not durably materialized");
			}
			resolved = await this.#resolveSessionProfile?.(profile, request.cwd);
			const prior = await this.store.getBinding(request.agentInstanceId);
