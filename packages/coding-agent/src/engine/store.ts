...
export class EngineStore {
	...
	async getBinding(agentInstanceId: string): Promise<EngineBindingSnapshot | undefined> {
		const rows = (await this.#client.unsafe(
			`SELECT binding_id, agent_instance_id, execution_id, attempt_id, engine_agent_id, session_file,
			 profile_digest, state, engine_generation, binding_generation
	...
	async putBinding(binding: EngineBindingSnapshot): Promise<void> {
		await this.#client.unsafe(
			`INSERT INTO engine_runtime_bindings(
			 binding_id, agent_instance_id, execution_id, attempt_id, engine_agent_id, session_file,
			 profile_digest, state, engine_generation, binding_generation, updated_at
			 ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			 ON CONFLICT(agent_instance_id) DO UPDATE SET
			 binding_id=excluded.binding_id, execution_id=excluded.execution_id, attempt_id=excluded.attempt_id,
	...
	async appendEvent(event: Omit<EngineEvent, "eventId" | "seq" | "createdAt">): Promise<EngineEvent> {
		...
		const rows = (await this.#client.unsafe(
			`INSERT INTO engine_event_outbox(
			 seq, agent_instance_id, execution_id, attempt_id, engine_generation,
			 binding_generation, kind, payload, created_at
	...
	async pendingEvents(limit = 100): Promise<EngineEvent[]> {
		const rows = (await this.#client.unsafe(
			`SELECT event_id, seq, agent_instance_id, execution_id, attempt_id, engine_generation,
			 binding_generation, kind, payload, created_at
