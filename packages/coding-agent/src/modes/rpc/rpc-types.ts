...
export type RpcUiEditorPasteChoice = "wrapped" | "localFile" | "inline";

export type RpcUiEditorPasteResult =
	| { status: "applied"; editor: RpcUiEditorState; fallback?: boolean }
	| {
			status: "pending-choice";
			editor: RpcUiEditorState;
			pendingId: string;
			lineCount: number;
			choices: RpcUiEditorPasteChoice[];
	  };

export type RpcUiEditorSubmitDisposition =
 | "noop"
 | "abort-on-empty-running-input"
 | "focused-agent-chat"
 | "continue"
 | "queued-messages"
 | "builtin"
...
export interface RpcUiEditorSubmitResult {
 accepted: boolean;
 disposition: RpcUiEditorSubmitDisposition;
 editor: RpcUiEditorState;
 operationId?: string;
}
...
export type RpcResponse =
 ...
 | {
			id?: string;
			type: "response";
			command: "ui_editor_paste";
			success: true;
			data: RpcUiEditorPasteResult;
	  }
 | {
			id?: string;
			type: "response";
			command: "ui_editor_paste_resolve";
			success: true;
			data: RpcUiEditorPasteResult;
	  }
 | {
