...
export function renderSetupOutro(width: number, height: number, elapsedMs: number): string[] {
	...
	const title = theme.bold(
		theme.fg("success", `${theme.status.success} ${i18n.t("setup.outro.saved", "Setup saved")}`),
	);
	const subtitle = theme.fg("muted", i18n.t("setup.outro.handoff", "Handing off to the normal CLI…"));
