...
function loginUrlLink(url: string): string {
	return `\x1b]8;;${url}\x07${localizeUiText("Open login URL")}\x1b]8;;\x07`;
}

function loginCopyHint(): string {
	return theme.fg("dim", localizeUiText("(clipboard copy attempted; Alt+C retries)"));
}
...
export class SignInTab implements SetupTab {
	...
	render(width: number, maxLines?: number): readonly string[] {
		const lines: string[] = [];
		if (this.#loggingInProvider) {
			lines.push(theme.bold(`${localizeUiText("Signing in to")} ${this.#loggingInProvider}`));
		} else {
			...
			if (maxLines === undefined || maxLines >= 17 + 2) {
				lines.push(
					theme.fg("muted", localizeUiText("Pick a provider to sign in — you can connect more than one.")),
					"",
				);
			}
		...
		if (this.#authUrl) {
			lines.push(
				theme.fg("accent", `${localizeUiText("Browser login:")} ${loginUrlLink(this.#authUrl)} ${loginCopyHint()}`),
				...urlLines.slice(0, 2),
			);
			if (this.#authLaunchUrl) {
				lines.push(
					theme.fg("dim", `${localizeUiText("Local shortcut (this machine only):")} ${this.#authLaunchUrl}`),
				);
			}
		}
		if (this.#prompt) {
	...
	async #login(providerId: string): Promise<void> {
		...
		this.#statusLines = [theme.fg("dim", localizeUiText("Starting OAuth flow…"))];
		...
		try {
			await this.#authStorage.login(providerId as OAuthProvider, {
				...
				onAuth: info => {
					...
					if (useManualInput) {
						this.#statusLines.push(
							theme.fg("dim", localizeUiText("Paste the returned code or redirect URL when prompted.")),
						);
					}
					void this.#copyAuthUrl();
				...
				onManualCodeInput: () =>
					this.#showPrompt({ message: localizeUiText("Paste the authorization code (or full redirect URL):") }),
			});
			...
			this.#statusLines = [
				...
				theme.fg("dim", `${localizeUiText("Credentials saved to")} ${getAgentDbPath()}`),
			];
			this.#authUrl = undefined;
			this.#authLaunchUrl = undefined;
			...
			if (this.#loginAbort?.signal.aborted) {
				this.#statusLines = [theme.fg("dim", localizeUiText("Login cancelled."))];
				this.#authUrl = undefined;
				this.#authLaunchUrl = undefined;
			} else {
				const message = error instanceof Error ? error.message : String(error);
				this.#statusLines = [
					theme.fg("error", `${localizeUiText("Login failed:")} ${message}`),
					theme.fg("dim", localizeUiText("Choose another provider or press Esc to continue.")),
				];
				this.#authUrl = undefined;
				this.#authLaunchUrl = undefined;
			}
