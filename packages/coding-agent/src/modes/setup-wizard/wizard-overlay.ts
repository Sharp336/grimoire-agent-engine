...
import { PI_LOGO } from "../components/welcome";
...
export class SetupWizardComponent implements Component, OverlayFocusOwner {
	...
	render(width: number): readonly string[] {
		const width_ = Math.max(1, width);
		const height = Math.max(1, this.ctx.ui.terminal.rows);
		return this.#fitToScreen(this.#phase === "done" ? [] : this.#renderScene(width_, height), width_, height);
	}
	...
	#renderScene(width: number, height: number): string[] {
		...
		const logo = PI_LOGO.map(line => theme.fg("accent", line));
		const header = [
			"",
