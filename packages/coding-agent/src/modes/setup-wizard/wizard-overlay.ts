...
export class SetupWizardComponent implements Component, OverlayFocusOwner {
	...
	#renderScene(width: number, height: number): string[] {
		...
		const title = this.#activeScene?.title ?? scene?.title ?? localizeUiText("Setup");
		...
		const header = [
			...
			centerLine(
				theme.fg("muted", `${localizeUiText("Setup step")} ${this.#sceneIndex + 1} / ${this.scenes.length}`),
				width,
			),
			"",
		...
		const footer = [
			"",
			centerLine(
				theme.fg("dim", localizeUiText("↑/↓ select · enter confirm · esc skip · ctrl+c exit setup")),
				width,
			),
		];
