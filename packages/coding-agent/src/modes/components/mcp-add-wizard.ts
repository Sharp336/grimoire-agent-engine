...
export class MCPAddWizard extends Container {
	...
	#renderAuthMethodStep(): void {
		...
		const options = [
			{ value: "oauth" as const, label: i18n.t("ui.oauthFlow", "OAuth flow (web-based)"), desc: i18n.t("ui.opensBrowser", "(opens browser)") },
			{ value: "manual" as const, label: i18n.t("ui.manualApiKey", "Manual API key/token"), desc: i18n.t("ui.pasteOrShellCmd", "(paste or use shell command)") },
		];
	...
	async #launchOAuthFlow(): Promise<void> {
		...
		try {
			...
			if (cancelled) {
				...
				this.#contentContainer.addChild(
					new Text(theme.fg("muted", i18n.t("ui.tip.authFaster", "Tip: Complete authorization faster next time")), 0, 0),
				);
			} else if (errorMsg.includes("Invalid OAuth URLs")) {
				this.#contentContainer.addChild(
					new Text(theme.fg("muted", i18n.t("ui.tip.oauthUrls", "Tip: Check that the OAuth URLs are correct")), 0, 0),
				);
			} else if (errorMsg.includes("ECONNREFUSED")) {
				this.#contentContainer.addChild(
					new Text(theme.fg("muted", i18n.t("ui.tip.oauthAccessible", "Tip: Verify the OAuth server is accessible")), 0, 0),
				);
			}
