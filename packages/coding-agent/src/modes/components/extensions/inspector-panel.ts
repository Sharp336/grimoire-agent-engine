...
export class InspectorPanel implements Component {
	...
	render(width: number): readonly string[] {
		...
		lines.push(`  ${this.#getStatusBadge(ext)}`);
		lines.push("");
	...
	#getStatusBadge(ext: Extension): string {
		switch (ext.state) {
			case "active": {
				const suffix =
					ext.activationMode !== "binary" &&
					ext.activationTarget === "project" &&
					ext.activationState === "inherit"
						? " (inherit)"
						: "";
			...
			case "disabled": {
				if (
					...
					return theme.fg("dim", `${theme.status.disabled} Disabled (inherit)`);
				}
				const reasonText =
			...
			case "shadowed":
				return theme.fg(
					"warning",
					`${theme.status.shadowed} Shadowed${ext.shadowedBy ? ` by ${ext.shadowedBy}` : ""}`,
				);
		}
	}
}
