...
export function buildHotkeysMarkdown(bindings: HotkeysMarkdownBindings): string {
	return [
		...
		"",
		"**Mouse / Fullscreen viewport**",
		"| Gesture | Action |",
		"|---------|--------|",
		"| `Wheel` / trackpad | Scroll the transcript |",
		"| Drag, then release | Select and copy the rendered text under the highlight |",
		"| `Alt+click` a message or card | Copy that block's complete source without activating it |",
		"| Double-click prose | Copy the whole message; activatable cards instead perform two ordinary toggles |",
