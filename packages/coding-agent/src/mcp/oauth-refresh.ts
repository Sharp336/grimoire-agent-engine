...
export function validateTokenUrl(
  ...
  if (parsed.protocol !== "https:" && !(parsed.protocol === "http:" && isLocalhost)) {
    throw new Error(
      `Token URL must use HTTPS (got ${parsed.protocol}). HTTP is only allowed for localhost.`,
    );
  }

  const tokenOrigin = parsed.origin;
  const originsMatch =
    tokenOrigin === serverOrigin ||
    (authServerOrigin !== undefined && tokenOrigin === authServerOrigin);

  if (!originsMatch) {
    const allowed = authServerOrigin
      ? `${serverOrigin} or ${authServerOrigin}`
      : serverOrigin;
    throw new Error(
      `Token URL origin ${tokenOrigin} does not match expected origin(s): ${allowed}`,
    );
  }
  ...
  for (const pattern of PRIVATE_IP_PATTERNS) {
    if (pattern.test(hostname)) {
      throw new Error(
        `Token URL hostname ${hostname} resolves to a private/reserved address`,
      );
    }
  }
}
...
export async function refreshMCPOAuthToken(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `OAuth token refresh failed (HTTP ${response.status}): ${safeMsg}`,
    );
  }
