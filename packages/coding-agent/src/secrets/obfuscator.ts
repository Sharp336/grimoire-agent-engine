...
export interface SecretEntry {
	...
	flags?: string;
	friendlyName?: string;
	/** Provenance for the display marker (D2): where this secret came from.
	 * Defaults: "secrets-yml" for constructor entries, "user-typed" for
	 * runtime `addSecret` calls, the provider name for `/redact`. */
	source?: string;
}
...
export class SecretObfuscator {
	...
	/** Provenance per placeholder (D2): source + registration time. Display-path only. */
	readonly #provenance = new Map<string, { source: string; registeredAt: number }>();
	...
	constructor(entries: SecretEntry[], key: string = defaultPlaceholderKey()) {
		...
		for (const entry of entries) {
			...
			const mode = entry.mode ?? "obfuscate";
				...
				this.#provenance.set(placeholder, { source: entry.source ?? "secrets-yml", registeredAt: Date.now() });
				index++;
				hasRealSec = true;
			}
		}
	...
	/**
	 * Register a new secret at runtime. Fail-closed: throws on invalid regex
	 * rather than silently skipping (unlike the constructor).
	 *
	 * NOTE (R6): This is a convenience for the operator to register secrets
	 * mid-session. It is NOT a security boundary against a malicious agent —
	 * the agent's Bash/Read can still reach secrets in process memory. The
	 * adversarial guarantee requires the broker (Tier-2: spawn-by-broker +
	 * closed PATH allowlist).
	 */
	/** ANSI bold magenta prefix, for display-path restoration. */
	static readonly DISPLAY_PREFIX = "\x1b[1;35m";
	/** ANSI reset + "[redacted from LLM]" marker, for display-path restoration. */
	static readonly DISPLAY_SUFFIX = "\x1b[0m [redacted from LLM]";

	/**
	 * Restore placeholders for DISPLAY only. Wraps each restored value in
	 * bold-magenta ANSI + "[redacted from LLM]" marker so the operator can
	 * discern what's redacted from the LLM. Tool-call path uses deobfuscate()
	 * (raw value). Mirrors #deobfuscate's recursion policy.
	 */
		...
		for (;;) {
			...
			const next = result.replace(PLACEHOLDER_RE, match => {
				const mapped = this.#lookupLiveAlias(match);
								if (mapped !== undefined) {
					shouldContinue ||= mapped.recursive;
					const provenance = this.#provenanceFor(match);
					return `${SecretObfuscator.DISPLAY_PREFIX}${mapped.secret}\x1b[0m [redacted from LLM — source:${provenance.source}, visible-to:human, ttl:session]`;
				}
				return match;
			});
			if (next === result || !shouldContinue || !next.includes("$$")) return next;
	 ...
	 * rather than silently skipping (unlike the constructor).
	 *
	 * NOTE (R6): operator convenience, NOT a security boundary against a
	 * malicious agent. The adversarial guarantee requires the broker (Tier-2:
	 * spawn-by-broker + closed PATH allowlist).
	 */
	/** Look up provenance for a placeholder (direct, then unprefixed alias). */
	#provenanceFor(placeholder: string): { source: string; registeredAt: number } {
		const direct = this.#provenance.get(placeholder);
		if (direct !== undefined) return direct;
		const unprefixed = placeholderWithoutFriendlyName(placeholder);
		if (unprefixed !== undefined) {
			const aliased = this.#provenance.get(unprefixed);
			if (aliased !== undefined) return aliased;
		}
		return { source: "unknown", registeredAt: 0 };
	}
	...
	addSecret(entry: SecretEntry): void {
		...
		if (entry.type === "plain") {
			this.#configuredSecretValues.add(entry.content);
						if (mode === "obfuscate") {
				if (entry.content.length < MIN_OBFUSCATE_SECRET_LEN) return;
				const index = this.#nextIndex++;
				const placeholder = this.#createPlaceholder(entry.content, entry.friendlyName);
				this.#plainMappings.set(entry.content, index);
				this.#obfuscateMappings.set(index, { secret: entry.content, placeholder });
				this.#generatedPlaceholders.add(placeholder);
				this.#provenance.set(placeholder, { source: entry.source ?? "user-typed", registeredAt: Date.now() });
			}
		} else {
			const regex = compileSecretRegex(entry.content, entry.flags);
			this.#regexEntries.push({
...
export function deobfuscateAssistantContent(
	...
	const result = content.map((block): AssistantMessage["content"][number] => {
		if (block.type === "text") {
			const text = obfuscator.deobfuscateForDisplay(block.text);
