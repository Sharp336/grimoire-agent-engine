...
async function closeCmuxTuiBrowserResource(
	...
	const close = (): Promise<unknown> =>
		client.request(
			"browser.close",
			{ machine: machineId, session: sessionId, browser: browserId },
			{ mutation: true, timeoutMs: remaining() },
		);
	try {
		await close();
	} catch (error) {
	...
	try {
		if (browserId === undefined) {
			...
			if (
				...
				throw new ToolError("cmux-tui tab.create_browser did not return stable workspace/browser IDs");
			}
		...
		if (ownsBrowser && browserId) {
			await closeCmuxTuiBrowserResource(browser.client, browserId, opts.timeoutMs).catch(() => undefined);
		}
		throw error;
	}
}
			...
			if (tab.cmuxOwnsSurface) {
				try {
					await waitForTabCleanup(
						...
						closeCmuxTuiBrowserResource(tab.browser.client, tab.targetId, timeoutMs),
					);
					// Closing the owned browser proves the attachment is gone too, so an
