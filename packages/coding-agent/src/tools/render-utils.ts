...
export function resolveImageOptions(): { maxWidthCells: number; maxHeightCells?: number } {
	// Renderers can run before `Settings.init()` (early TUI paint, tests); fall
	// back to the schema defaults rather than throwing on the global proxy.
	const ready = isSettingsInitialized();
	const maxWidthCells = ready ? settings.get("tui.maxInlineImageColumns") : getDefault("tui.maxInlineImageColumns");
	const rowSetting = Math.max(
		0,
		ready ? settings.get("tui.maxInlineImageRows") : getDefault("tui.maxInlineImageRows"),
	);
