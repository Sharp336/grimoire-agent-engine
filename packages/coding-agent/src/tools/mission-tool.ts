...
/**
 * Mission accessors exposed through the same ToolSession construction path that
 * carries `getGoalRuntime`. Declared here (optional) so the tool compiles
 * independently of the AgentSession wiring pass, and so a session without a
 * mission simply yields `undefined`.
 */
export interface MissionSessionAccessors {
	/** Runtime for the active top-level session, or undefined outside a mission. */
	getMissionRuntime?: () => MissionRuntimeContract | undefined;
	/** Authoritative snapshot without going through the runtime. */
...
/** Persisted render payload. The snapshot is the single source the renderer reads. */
export interface MissionToolDetails {
	op: MissionToolOp;
	mission: MissionState | null;
	/** Handoff produced by this call (`run_next`); null when nothing was runnable. */
...
function requirePlan(params: MissionToolInput): MissionPlan {
	if (!params.plan) {
		throw new ToolError("set_plan requires a complete `plan`.");
	}
	return params.plan;
}

function requireDecision(params: MissionToolInput): {
	decision: MissionHandoffDecision;
	messageToWorker?: string;
} {
	if (!params.decision) {
		throw new ToolError("resolve_handoff requires a `decision`.");
	}
...
function requireRemediation(params: MissionToolInput): MissionRemediationFeatureSpec[] {
	...
	if (!features || features.length === 0) {
		throw new ToolError("revise_pending requires a nonempty `add_features`.");
	}
	return features;
}
...
/** Feature currently owning the run or the pending handoff, if any. */
...
/** One-line, tab-safe, width-bounded handoff gist. Never echoes worker evidence verbatim. */
export function missionHandoffSummary(
	...
	return replaceTabs(truncateToWidth(`${verdict}${suffix}: ${handoff.summary.trim()}`, maxWidth));
}
...
/** Machine-facing text: a short header the model can read plus the exact snapshot. */
function renderMissionText(state: MissionState | null, handoff: MissionHandoff | null | undefined): string {
	...
	if (handoff !== undefined) {
		lines.push(`Handoff this turn: ${handoff === null ? "none (nothing runnable)" : JSON.stringify(handoff)}`);
	}
...
export class MissionTool implements AgentTool<typeof missionSchema, MissionToolDetails> {
	...
	/** Reads are `read`, plan/handoff mutations are `write`, dispatching a worker is `exec`. */
	...
	async execute(
		...
		if (!runtime) {
			throw new ToolError("No mission is active.");
		}
...
export const missionToolRenderer = {
	...
	renderResult(
		...
		const lines: string[] = [
			uiTheme.italic(uiTheme.fg("muted", `"${truncateToWidth(mission.goal.trim(), TRUNCATE_LENGTHS.LONG)}"`)),
		];
		...
		if (summary) {
			sections.push({ label: "Handoff", lines: [uiTheme.fg("muted", summary)] });
		}
