 ...
 *
 * `settings` is required (never the global proxy): the guard has to honor the
 * *session's* `edit.blockAutoGenerated`, which for SDK/subagent sessions is an
 * isolated instance that differs from the global one. Making it mandatory is
 * also what keeps a new call site from silently falling back to global state —
 * the type checker enumerates them.
 *
 * @param absolutePath - Absolute path to the file
 * @param displayPath - Path to show in error messages (relative or as provided)
 * @param settings - Settings of the session performing the edit
 */
export async function assertEditableFile(absolutePath: string, displayPath: string, settings: Settings) {
 ...
 * @param settings - Settings of the session performing the edit (see {@link assertEditableFile})
 */
export function assertEditableFileContent(content: string, displayPath: string, settings: Settings): void {
	if (!settings.get("edit.blockAutoGenerated")) {
		return;
	}
