...
function resolveConfiguredRolePattern(
	...
	const configured = (roleLookup ? roleLookup(role) : settings?.getModelRole(role))?.trim();
	const configuredDefault = (
		roleLookup ? roleLookup(DEFAULT_MODEL_ROLE) : settings?.getModelRole(DEFAULT_MODEL_ROLE)
	)?.trim();
