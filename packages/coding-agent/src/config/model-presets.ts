...
export interface ResolvedModelPresetOverlay {
	id: ModelPresetId;
	info: ModelPresetInfo;
	roles: ResolvedModelPresetRole[];
}
...
function stripTerminalControls(value: string): string {
	return value.replace(TERMINAL_CONTROL_CHARS, "");
}

function isDangerousPresetKey(value: string): boolean {
	return DANGEROUS_PRESET_KEYS.has(value);
}
...
function getBudgetPreset(settings: Settings): ModelPresetRoleMap {
	...
	if (settings.getBaseModelRole("vision")) {
...
function stringRoleMap(value: unknown): ModelPresetRoleMap | undefined {
	...
	const roles = Object.create(null) as ModelPresetRoleMap;
	for (const [rawRole, rawSelector] of Object.entries(value)) {
		const role = stripTerminalControls(rawRole).trim();
		if (!role || isDangerousPresetKey(role) || typeof rawSelector !== "string") continue;
		const selector = stripTerminalControls(rawSelector).trim();
		if (selector) {
			roles[role] = selector;
		}
	}
...
function resolveModelPresetRoles(
	settings: Settings,
	modelRegistry: ModelRegistry,
	availableModels: readonly Model<Api>[],
	id: ModelPresetId,
	skipRoles?: ReadonlySet<string>,
	roleOverrides?: Readonly<Record<string, string>>,
): ResolvedModelPresetOverlay {
	...
	for (const role of orderPresetRoles(roleMap)) {
		...
		const resolved = resolveModelRoleValue(selector, available, {
			...
			matchPreferences,
			modelRegistry,
			roleLookup: r => roleOverrides?.[r] ?? settings.getBaseModelRole(r),
		});
		if (!resolved.model) {
...
export function resolveModelPreset(
	...
	const resolved = resolveModelPresetRoles(settings, modelRegistry, availableModels, id);
	const roleMap = getModelPresetRoleMap(settings, id);
	const defaultRole = resolved.roles.find(role => role.role === "default");
	if (!defaultRole) {
...
export function resolveModelPresetOverlay(
	settings: Settings,
	modelRegistry: ModelRegistry,
	availableModels: readonly Model<Api>[],
	id: ModelPresetId,
	...
	return resolveModelPresetRoles(settings, modelRegistry, availableModels, id, skipRoles, roleOverrides);
}
...
export function modelPresetsFromUnknown(value: unknown): ModelPresetOverrides {
	...
	const presets = Object.create(null) as ModelPresetOverrides;
	for (const [rawKey, item] of Object.entries(value)) {
		const id = stripTerminalControls(rawKey).trim();
		const roles = stringRoleMap(item);
		if (id && !isDangerousPresetKey(id) && roles) {
			presets[id] = roles;
		}
	}
	return presets;
