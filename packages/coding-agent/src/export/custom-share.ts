 ...
 *
 * Allows users to define a custom share handler at ~/.oh-omp/agent/share.ts
 * that will be used instead of the default GitHub Gist sharing.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { getAgentDir } from "@oh-my-pi/pi-utils";
import { requestCodeLoadAccess } from "../security/access";
 ...
 */
export async function loadCustomShare(
	cwd: string,
	approvalUi?: {
...
): Promise<LoadedCustomShare | null> {
