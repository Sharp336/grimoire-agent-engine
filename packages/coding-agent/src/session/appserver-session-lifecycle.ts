...
const LOCK_RELEASE_RETRY_MS = 5;
const RETAINED_LOCK_RELEASE_RETRY_MIN_MS = 25;
const RETAINED_LOCK_RELEASE_RETRY_MAX_MS = 1_000;
...
export class SessionLifecycleCommittedError extends Error {
	readonly committed = true;
	readonly operation: "archive" | "restore" | "delete";
	readonly cause: unknown;

	constructor(operation: "archive" | "restore" | "delete", cause: unknown) {
		super(`session ${operation} committed but lock release failed`, { cause });
		this.name = "SessionLifecycleCommittedError";
		this.operation = operation;
		this.cause = cause;
	}
}

async function releaseSessionLock(lock: SessionLockHandle): Promise<void> {
	let failure: unknown;
	for (let attempt = 0; attempt < LOCK_RELEASE_ATTEMPTS; attempt++) {
		try {
			lock.release();
			if (!lock.released) throw new Error("session lock release returned without relinquishing ownership");
			return;
		} catch (error) {
			failure = error;
			if (attempt < LOCK_RELEASE_ATTEMPTS - 1) await Bun.sleep(LOCK_RELEASE_RETRY_MS);
		}
	}
	throw failure;
}
...
export class AppserverSessionLifecycleStore {
	...
	readonly #retainedLockReleases = new Set<SessionLockHandle>();
	...
	async #releaseLock(lock: SessionLockHandle): Promise<void> {
		try {
			await releaseSessionLock(lock);
		} catch (error) {
			this.#retainLockForRelease(lock);
			throw error;
		}
	}

	#retainLockForRelease(lock: SessionLockHandle): void {
		if (lock.released || this.#retainedLockReleases.has(lock)) return;
		this.#retainedLockReleases.add(lock);
		void this.#retryRetainedLockRelease(lock);
	}

	async #retryRetainedLockRelease(lock: SessionLockHandle): Promise<void> {
		let delay = RETAINED_LOCK_RELEASE_RETRY_MIN_MS;
		try {
			while (!lock.released) {
				...
				try {
					...
					// The lifecycle error already reported the failed release. Keep the
					// owned handle reachable and retry until it actually relinquishes.
				}
				delay = Math.min(delay * 2, RETAINED_LOCK_RELEASE_RETRY_MAX_MS);
			}
		} finally {
			this.#retainedLockReleases.delete(lock);
		}
	}
	...
	async archiveSession(id: SessionId, archivedAt: string, sessionPath: string): Promise<void> {
		...
		try {
			...
			if (committed) throw new SessionLifecycleCommittedError("archive", error);
			throw failure === undefined
				? error
				: new AggregateError([failure, error], "session archive and lock release failed");
		}
		if (failure !== undefined) throw failure;
	}

	async restoreSession(id: SessionId, sessionPath: string): Promise<void> {
		const { transcript } = await this.#canonicalSessionPath(sessionPath);
		const lock = this.#acquireLock(transcript);
		let committed = false;
		let failure: unknown;
		...
		try {
			...
			if (committed) throw new SessionLifecycleCommittedError("restore", error);
			throw failure === undefined
				? error
				: new AggregateError([failure, error], "session restore and lock release failed");
		}
		if (failure !== undefined) throw failure;
	}
	...
	async deleteSession(id: SessionId, sessionPath: string): Promise<void> {
		...
		try {
			...
			if (committed) throw new SessionLifecycleCommittedError("delete", error);
			throw failure === undefined
				? error
				: new AggregateError([failure, error], "session deletion and lock release failed");
		}
		if (failure !== undefined) throw failure;
		if (!committed) return;
		try {
