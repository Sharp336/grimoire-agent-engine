...
function emitTerminalOutputFrames(
	onOutput: DesktopTerminalRequest["onOutput"],
	stream: "stdout" | "stderr",
	data: string,
): void {
	if (!onOutput || !data) return;
	if (Buffer.byteLength(data, "utf8") <= MAX_OUTPUT_BYTES) {
	...
	for (let index = 0; index < data.length; ) {
		...
		const codeUnits = codePoint > 0xffff ? 2 : 1,
			codePointBytes = codePoint <= 0x7f ? 1 : codePoint <= 0x7ff ? 2 : codePoint <= 0xffff ? 3 : 4;
		if (frameBytes + codePointBytes > MAX_OUTPUT_BYTES && index > start) {
			onOutput(stream, data.slice(start, index));
			start = index;
			frameBytes = 0;
		}
	...
	if (start < data.length) onOutput(stream, data.slice(start));
}
...
export class CodingAgentDesktopAuthority {
	...
	#getRoot(sessionId = this.#context.sessionManager.getSessionId()): string {
		try {
			...
			if (!nodePath.isAbsolute(root)) throw protocolError("FORBIDDEN");
			return root;
		} catch {
			throw protocolError("FORBIDDEN");
		}
	...
	async filesPatch(request: DesktopPatchRequest, context?: OperationContextLike): Promise<Record<string, unknown>> {
		...
		const root = this.#getRoot(sessionId),
			absolute = nodePath.resolve(root, path);
		const fs: FileSystem = {
	...
	async #terminalCwd(root: string, raw: string | undefined): Promise<string> {
		...
		try {
			const candidate = nodePath.resolve(root, safeRelativePath(raw));
			const canonical = await fsp.realpath(candidate);
			const escaped = nodePath.relative(root, canonical);
			if (escaped.startsWith("..") || nodePath.isAbsolute(escaped)) throw protocolError("FORBIDDEN");
			return canonical;
		} catch {
			throw protocolError("FORBIDDEN");
		}
	...
	async openTerminal(
		...
		await fsp.access(shell, fs.constants.X_OK);
		if (
		...
		void pty
			.start(
				...
				(error, chunk) => {
					if (handle.closed || error || !chunk) return;
					emitTerminalOutputFrames(request.onOutput, "stdout", chunk);
				},
			)
			.then(result => {
	...
	#metadata(value: unknown, depth = 0, state = { nodes: 0 }): unknown {
		...
		if (typeof value === "string") {
			...
			return nodePath.isAbsolute(value) ? "<redacted-path>" : value;
		}
		...
		for (const [key, child] of Object.entries(value)) {
			if (Object.keys(result).length >= 256 || SECRET_KEY.test(key) || nodePath.isAbsolute(key)) continue;
