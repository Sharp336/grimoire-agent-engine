...
export interface CommittedSessionFork {
	...
	/** Whether the child inherited committed parent conversation messages. */
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#readOnlySideTransforms = config.readOnlySideTransforms ?? {
			...
			transformProviderContext: (context, model) =>
				this.#transformProviderContext ? this.#transformProviderContext(context, model) : context,
		};
	...
	async runReadOnlySideTurn(args: {
		...
		const options = this.prepareSimpleStreamOptions(
			{
				...
				serviceTier: this.#effectiveServiceTier(request.model),
				signal: args.signal,
			},
		...
		const serviceTier = this.#effectiveServiceTier(request.model);
