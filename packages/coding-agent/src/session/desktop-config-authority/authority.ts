...
import type { Settings, SettingsDesktopSnapshot } from "../../config/settings";
import { SETTINGS_SCHEMA, type SettingPath } from "../../config/settings-schema";
import type { AgentRegistry } from "../../registry/agent-registry";
import { BUILTIN_SLASH_COMMAND_DEFS } from "../../slash-commands/builtin-registry";
import { loadBundledAgents } from "../../task/agents";
import { BUILTIN_TOOLS, HIDDEN_TOOLS } from "../../tools/index";
...
type SettingScope = "global" | "project" | "cli" | "read-only";
...
function pathSafe(value: string): string {
	return path.isAbsolute(value) ? "<redacted-path>" : value;
}
...
export class DesktopConfigAuthority {
	...
	#validateEdit(edit: SettingsWriteEdit): {
		...
		if (scope !== "global") throw new Error(`unsupported settings scope: ${scope}`);
	...
	#applyEdit(edit: SettingsWriteEdit, normalized: { path: SettingPath; scope: "global"; reset: boolean }): void {
		if (normalized.reset) {
