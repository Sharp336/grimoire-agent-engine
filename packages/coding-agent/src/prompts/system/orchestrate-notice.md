...
3. **Dispatch phase.** Launch all parallel `task` subagents in one message, then keep doing useful orchestration work while async results arrive automatically. Use `job` only when every runnable next step depends on missing subagent output.
