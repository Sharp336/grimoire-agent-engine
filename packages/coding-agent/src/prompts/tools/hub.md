...
- **`wait`** with `name` REQUIRES `id` — the daemon id returned by `start`/`restart`/`describe` (generation binding). Missing or stale `id` (a restart rotated it), a wrong owner, or an unknown daemon are refused immediately — never a long default wait. `pattern` is a JavaScript `RegExp` compiled with the `u` flag (no inline modifiers such as `(?i)`).
  ...
  - A restart rotates the daemon id — re-read it from `start`/`restart`/`describe`/`ps` before re-waiting. If the bound generation exits into a restart while the wait is in flight, the wait is refused immediately (never attached to the replacement process); re-bind with the current id.
  - A stale broker (a previous CLI version kept alive by persistent/detached daemons) refuses id-bound and auto waits with `upgrade-required` — it is never restarted by the client; legacy-shaped waits (explicit `for`, no `id`) still work until the old broker is stopped.
