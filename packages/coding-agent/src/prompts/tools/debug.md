...
- `action: "launch"` starts a session; `program` is required, `adapter` optional (auto-selected from target path and workspace).
  For Python scripts, set `adapter: "debugpy"` and `program` to the target `.py` file. Do not set `program: "python"`; put interpreter/script flags in `args`.
...
`debug(action: "launch", adapter: "debugpy", program: "scripts/repro.py", args: ["--flag"], cwd: ".")`
