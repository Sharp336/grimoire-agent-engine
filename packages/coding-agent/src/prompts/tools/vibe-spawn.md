...
Returns immediately with the session id; the turn's preview is delivered automatically when the worker finishes. Complete worker turns in this parent workspace execute FIFO to prevent overlapping bash/edit/write mutations; queue time does not consume the turn's timeout budget. Different parent scopes may run concurrently. When artifact persistence succeeds, a settled result exposes an immutable `fullOutputUrl` (`agent://<id>/turn-<n>`); `agent://<id>` remains the latest persisted-output alias. A turn that fails before producing a result may have no `fullOutputUrl`. Do not wait unless blocked — keep directing queued sessions, but do not expect same-workspace turns to run concurrently.
