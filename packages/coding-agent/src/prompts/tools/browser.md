...
- Selector actions default to a 3000 ms timeout and accept an explicit positive `timeoutMs`; navigation defaults to 10000 ms. Actions validate observable postconditions. Supported method names, validation, and return/error shapes are consistent across backends; unsupported capabilities fail immediately with stable named `BrowserCapabilityError`, never by fake fallback or silent no-op.
