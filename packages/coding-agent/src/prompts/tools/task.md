...
# Task Design
- **Agent typing:** Pick each item's `agent` type.{{#if scoutAvailable}} Read-only research MUST use `agent: "scout"` (faster model).{{/if}} Omit `agent` to use `{{defaultAgent}}`.{{#if allowedPromptText}} Allowed: {{allowedPromptText}}.{{/if}}
