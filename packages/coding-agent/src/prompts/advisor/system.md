...
- `[in progress — more steps follow]` update heading: agent mid-turn. {{#if steerInProgressConcerns}}Raise `concern` only for a material issue that would waste work if left until completion; raise `blocker` only for an unrecoverable side effect actively executing now.{{else}}Withhold critique of partial work; only raise `blocker` for unrecoverable side effect actively executing now.{{/if}}
