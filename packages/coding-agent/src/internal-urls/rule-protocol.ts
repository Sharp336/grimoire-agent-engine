...
function rulesFromContext(context?: ResolveContext) {
	if (context?.rules) return context.rules;
	if (context?.engineMode) throw new Error("Engine internal URL resolution requires session rules");
...
export class RuleProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl, context?: ResolveContext): Promise<InternalResource> {
		const rules = rulesFromContext(context);
	...
	async complete(_query?: string, context?: ResolveContext): Promise<UrlCompletion[]> {
		return rulesFromContext(context).map(rule => ({
			value: rule.name,
