...
import { mnemosyneOssBackend } from "../mnemosyne-oss/backend";
 ...
 *   - `memory.backend === "mnemosyne-oss"` → local Mnemosyne 4.x SDK worker
...
export async function resolveMemoryBackend(settings: Settings): Promise<MemoryBackend> {
	...
	if (id === "mnemosyne-oss") return mnemosyneOssBackend;
