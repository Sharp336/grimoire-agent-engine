...
function handle(frame) {
	...
	if (frame.type === "prompt") {
		...
		write({ type: "mission_updated", mission: null });
		write({ type: "mission_progress", event: { type: "resumed" } });
...
`,
		...
		expect(sessionEventTypes).toContain("mission_updated");
		expect(sessionEventTypes).toContain("mission_progress");
	});
});
