...
import * as videoGen from "@oh-my-pi/pi-coding-agent/tools/video-gen";
...
describe("applyProviderGlobalsFromSettings", () => {
	...
	it("reapplies valid web, image, and video provider globals from cwd-scoped settings", () => {
		...
		const videoOrderSpy = vi.spyOn(videoGen, "setVideoProviderOrder").mockImplementation(() => {});
		...
		applyProviderGlobalsFromSettings({
			get(
				...
				const values: Record<string, unknown> = {
					...
					"providers.videoOrder": ["openrouter", 42, "xai"],
				};
				return values[path];
			},
		});
		...
		expect(videoOrderSpy).toHaveBeenCalledWith(["openrouter", "xai"]);
	});
});
