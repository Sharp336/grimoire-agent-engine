...
describe("UndoTracker", () => {
	...
	it("skips capture when disabled", async () => {
		...
		await tracker.onAssistantTurnEnd();
		expect(sessionManager.getEntries().some(e => e.type === "custom" && e.customType === "undo-snapshot")).toBe(
			false,
		);
	});

	it("resolves undo boundaries across multiple turns", async () => {
		const tracker = new UndoTracker({
			sessionManager,
			projectRoot: projectDir,
			agentDataDir: agentDir,
		...
		await tracker.onUserTurnStart();
		const firstUserId = sessionManager.appendMessage(makeUserMessage("first"));
		await fs.writeFile(path.join(projectDir, "file.txt"), "v2");
		sessionManager.appendMessage(makeAssistantMessage("done1"));
		await tracker.onAssistantTurnEnd();

		await tracker.onUserTurnStart();
		sessionManager.appendMessage(makeUserMessage("second"));
		await fs.writeFile(path.join(projectDir, "file.txt"), "v3");
		await fs.writeFile(path.join(projectDir, "new.txt"), "created");
		sessionManager.appendMessage(makeAssistantMessage("done2"));
		await tracker.onAssistantTurnEnd();

		const boundaries = tracker.resolveUndo("2");
		expect(boundaries).toBeTruthy();
		expect(boundaries!.targetUserEntryId).toBe(firstUserId);
		expect(boundaries!.filesToRestore.sort()).toEqual(["file.txt", "new.txt"]);
		expect(boundaries!.userMessageText).toBe("first");
