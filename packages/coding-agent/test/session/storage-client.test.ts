...
const input = {
	operationId: "op-one", familyId: "family", generationId: "generation", firstSeq: 1,
	entries: [{ entryId: "entry", parentId: null, kind: "message", payload: { text: "hello" } }],
	durability: "required" as const, dependencies: [],
};
function envelope(requestId: string, fields: object, incarnation = 1) {
	return Response.json({ schema: "artel.storage.protocol.response.v1", version: "1.0", requestId, incarnation, ...fields });
}
function receipt(write: StorageWrite) {
	return { ...write, throughSeq: 1, admissionState: "admitted", appliedState: "applied", durabilityState: "durable", outcome: "success" };
}
function binding(port: number) {
	return { url: `http://127.0.0.1:${port}`, token: "0123456789012345", incarnation: 1, protocolHash: STORAGE_PROTOCOL_SCHEMA_HASH };
}
...
it("rejects overflow synchronously while retaining a separate read lane", async () => {
	...
	const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
		...
		return envelope(body.read.requestId, { familyId: "family", generationId: "generation", throughSeq: 0,
			durableThroughSeq: 0, liveThroughSeq: 0, events: [], nextCursor: null });
	} });
	try {
		...
		expect((await client.readRange({ familyId: "family", generationId: "generation", maxRecords: 1, maxBytes: 1024 })).events).toEqual([]);
		gate.resolve();
		await first;
