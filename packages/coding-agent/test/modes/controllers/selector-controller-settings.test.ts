...
describe("SelectorController prompt-affecting settings", () => {
	...
	it("refreshes every live lossless setting without rebinding the engine", () => {
		const refreshLcmSettings = vi.fn();
		const refreshLcmSettingsAndRebind = vi.fn();
		const ctx = {
			session: { refreshLcmSettings, refreshLcmSettingsAndRebind },
		} as unknown as InteractiveModeContext;
		const controller = new SelectorController(ctx);
		...
		expect(refreshLcmSettingsAndRebind).not.toHaveBeenCalled();
	});

	it("rebinds an engine change asynchronously and displays rebind failures", async () => {
		const refreshLcmSettingsAndRebind = vi.fn(async () => {
			throw new Error("rebind failed");
		});
		const showError = vi.fn();
		const ctx = {
			session: { refreshLcmSettings: vi.fn(), refreshLcmSettingsAndRebind },
			showError,
		} as unknown as InteractiveModeContext;
		const controller = new SelectorController(ctx);

		controller.handleSettingChange("context.engine", "lossless");

		expect(refreshLcmSettingsAndRebind).toHaveBeenCalledTimes(1);
		expect(showError).not.toHaveBeenCalled();
		await Promise.resolve();
		await Promise.resolve();
		expect(showError).toHaveBeenCalledWith("Failed to apply context engine: Error: rebind failed");
	});
