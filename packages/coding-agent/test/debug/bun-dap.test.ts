...
function requireBunAdapter(cwd: string): DapResolvedAdapter {
	const adapter = resolveAdapter("bun", cwd);
	if (!adapter) throw new Error("bundled Bun adapter did not resolve");
	return {
		...adapter,
		launchDefaults: {
...
describe("bundled Bun DAP adapter", () => {
	...
	it("binds breakpoints in imported TypeScript files", async () => {
		...
		try {
			...
			await Bun.write(
				program,
				[
					"import { importedScale } from './src/imported.ts';",
					"console.log(importedScale({ label: 'alpha', value: 2 }));",
					"",
				].join("\n"),
			);
	...
	it("steps over TypeScript call sites by source line", async () => {
		...
		try {
			...
			expect(locationBody.breakpoints?.some(location => location.line === 8 && (location.column ?? 0) > 0)).toBe(
				true,
			);
	...
	it("falls back to source-map binding when direct TypeScript lines are erased", async () => {
		...
		try {
			...
			const erasedLines = Array.from(
				{ length: 40 },
				(_, index) => `interface Erased${index} { readonly value${index}: number; }`,
			);
			...
			await Bun.write(
				program,
				[
					"import { calculate } from './erased-lines.ts';",
					"const boot = 0;",
					"console.log(calculate(21), boot);",
					"await new Promise(() => {});",
					"",
				].join("\n"),
			);
	...
	it("binds pre-launch imported TypeScript breakpoints after erased lines", async () => {
		...
		try {
			...
			const erasedLines = Array.from(
				{ length: 80 },
				(_, index) => `interface PrelaunchErased${index} { readonly value${index}: number; }`,
			);
			...
			await Bun.write(
				program,
				["import { calculate } from './erased-import.ts';", "console.log(calculate(21));", ""].join("\n"),
			);
