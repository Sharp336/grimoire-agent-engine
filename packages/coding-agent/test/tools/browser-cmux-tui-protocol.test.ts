...
describe("cmux-tui browser resources", () => {
	...
	it("routes public BrowserTool ownership by explicit stable browser ID", async () => {
		...
		await withSocketServer(
			(request, socket, server) => {
				...
				if (request.operation === "browser.close") {
					...
					server.writeResponse(socket, request, {
						value: {},
						generation: "g",
						revision: String(closes.length + 1),
						replayed: false,
					});
					return;
				}
			...
			async server => {
				...
				try {
					const tool = new BrowserTool(browserToolSession());
					await tool.execute("borrowed-open", {
						action: "open",
					...
					await tool.execute("owned-reuse", {
						action: "open",
						name: "public-owned",
						timeout: 2,
					});
					expect({ creates, closes }).toEqual({ creates: 1, closes: [] });
					await tool.execute("owned-explicit-reuse", {
						action: "open",
						name: "public-owned",
					...
					expect({ creates, closes }).toEqual({ creates: 1, closes: [] });
					await tool.execute("owned-close", { action: "close", name: "public-owned", timeout: 2 });
					...
					await tool.execute("borrowed-close", { action: "close", name: "public-borrowed", timeout: 2 });
					...
					expect({ creates, closes }).toEqual({ creates: 1, closes: [OWNED_BROWSER_ID] });
				} finally {
		...
		for (const firstCloseOutcome of ["absent", "present"] as const) {
			...
			await withSocketServer(
				(request, socket, server) => {
					...
					if (request.operation === "browser.get") {
						if (closeAttempts > 0) closeProbes++;
						if (!browserExists) {
							writeError("selector.not_found", "owned browser is absent");
							return;
						}
					...
					if (request.operation === "browser.close") {
						closeAttempts++;
						if (closeAttempts === 1) {
							browserExists = firstCloseOutcome === "present";
							socket.destroy();
							return;
						}
						browserExists = false;
				...
				async server => {
					...
					expect({ viewerReleases, closeAttempts, closeProbes, browserExists }).toEqual({
						viewerReleases: 2,
