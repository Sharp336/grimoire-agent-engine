...
describe("launch persona first system prompt", () => {
	...
	it("restores the baseline xd:// partition after leaving a persona", async () => {
		...
		// builtins (ast_edit/debug/browser) that a fresh non-persona session
		// would present under `xd://`.
		const baselineMounted = session.getBaselineMountedToolNames();
		expect(baselineMounted).toBeDefined();
		expect(baselineMounted).toEqual(expect.arrayContaining(["ast_edit", "debug", "browser"]));
		...
		expect(restoredMounted).toEqual(expect.arrayContaining(["ast_edit", "debug", "browser"]));
	});
