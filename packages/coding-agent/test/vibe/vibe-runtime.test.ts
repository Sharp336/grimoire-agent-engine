...
describe("vibe session registry", () => {
	...
	it("serializes complete worker turns FIFO within one parent workspace scope", async () => {
		const gates = new Map([
			["First", deferred()],
			["Second", deferred()],
		]);
		...
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			...
			await gates.get(options.id)!.promise;
			AgentRegistry.global().setStatus(options.id, "idle");
			order.push(`end:${options.id}`);
			active--;
		...
		const session = createSession({ manager });
		const registry = VibeSessionRegistry.global();
		const first = await registry.spawn(session, { cli: "fast", name: "First", prompt: "First turn." });
		const second = await registry.spawn(session, { cli: "good", name: "Second", prompt: "Second turn." });

		await pollUntil(() => order.length === 1);
		expect(order).toEqual(["start:First"]);
		expect(manager.getJob(second.jobId)?.queued).toBe(true);

		gates.get("First")!.resolve();
		await manager.getJob(first.jobId)!.promise;
		await pollUntil(() => order.includes("start:Second"));
	...
	it("runs worker turns from different parent scopes concurrently", async () => {
		...
		const manager = createManager();
		const registry = VibeSessionRegistry.global();
		const scopeA = createSession({ manager, parentSessionId: "parent-a" });
		const scopeB = createSession({ manager, parentSessionId: "parent-b" });
		const first = await registry.spawn(scopeA, { cli: "fast", name: "ScopeA", prompt: "A." });
		const second = await registry.spawn(scopeB, { cli: "good", name: "ScopeB", prompt: "B." });

		await pollUntil(() => active === 2);
		expect(maxActive).toBe(2);
		expect(started.toSorted()).toEqual(["ScopeA", "ScopeB"]);
		release.resolve();
		await Promise.all([manager.getJob(first.jobId)!.promise, manager.getJob(second.jobId)!.promise]);
	});

	it("settles a worker cancelled while waiting without executing it or leaking the workspace lane", async () => {
		...
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			...
			if (options.id === "Blocker") await firstGate.promise;
			if (options.id === "After") await thirdGate.promise;
			AgentRegistry.global().setStatus(options.id, "idle");
			return makeResult(options.id);
		});
		...
		const session = createSession({ manager });
		const registry = VibeSessionRegistry.global();
		const blocker = await registry.spawn(session, { cli: "fast", name: "Blocker", prompt: "Hold the lane." });
		await pollUntil(() => invoked.includes("Blocker"));
		const cancelled = await registry.spawn(session, { cli: "good", name: "Cancelled", prompt: "Never execute." });
		expect(manager.getJob(cancelled.jobId)?.queued).toBe(true);

		const killed = await registry.kill(session, "Cancelled");
		expect(killed.cancelledTurn).toBe(true);
		await manager.getJob(cancelled.jobId)!.promise;
		expect(manager.getJob(cancelled.jobId)?.status).toBe("cancelled");
		expect(invoked).toEqual(["Blocker"]);

		const after = await registry.spawn(session, { cli: "fast", name: "After", prompt: "Run after cancellation." });
		await flushMicrotasks();
		expect(invoked).toEqual(["Blocker"]);
		firstGate.resolve();
		await manager.getJob(blocker.jobId)!.promise;
		await pollUntil(() => invoked.includes("After"));
		thirdGate.resolve();
		await manager.getJob(after.jobId)!.promise;
		expect(invoked).toEqual(["Blocker", "After"]);
	});

	it("preserves immutable full output per turn while agent://id remains the latest alias", async () => {
		let artifactsDir = "";
		const writeTurnOutput = async (id: string, output: string): Promise<SingleResult> => {
			const outputPath = path.join(artifactsDir, `${id}.md`);
			await fs.writeFile(outputPath, output);
			return makeResult(id, { output, outputPath });
		};
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			...
			return writeTurnOutput(options.id, "first full output");
		});
		vi.spyOn(executorModule, "runSubagentFollowUpTurn").mockImplementation(options =>
			writeTurnOutput(options.id, "second full output"),
		);

		const manager = createManager();
		const session = createSession({ manager });
		const registry = VibeSessionRegistry.global();
		const first = await registry.spawn(session, { cli: "fast", name: "OutputWorker", prompt: "First." });
		await manager.getJob(first.jobId)!.promise;
		expect(await fs.readFile(path.join(artifactsDir, "OutputWorker.turn-1.md"), "utf8")).toBe("first full output");

		const second = await registry.send(session, { session: "OutputWorker", message: "Second." });
		await manager.getJob(second.jobId!)!.promise;
		expect(await fs.readFile(path.join(artifactsDir, "OutputWorker.turn-1.md"), "utf8")).toBe("first full output");
		expect(await fs.readFile(path.join(artifactsDir, "OutputWorker.turn-2.md"), "utf8")).toBe("second full output");
		expect(await fs.readFile(path.join(artifactsDir, "OutputWorker.md"), "utf8")).toBe("second full output");

		const waited = await registry.wait(session, { sessions: ["OutputWorker"], timeoutMs: 50 });
		const firstUrl = "agent://OutputWorker/turn-1";
		const secondUrl = "agent://OutputWorker/turn-2";
		expect((await InternalUrlRouter.instance().resolve(firstUrl)).content).toBe("first full output");
		expect((await InternalUrlRouter.instance().resolve(secondUrl)).content).toBe("second full output");
		expect((await InternalUrlRouter.instance().resolve("agent://OutputWorker")).content).toBe("second full output");
		expect(waited.settled[0]?.fullOutputUrl).toBe(secondUrl);
		expect(manager.getJob(first.jobId)?.resultText).toContain(`full-output="${firstUrl}"`);
		expect(manager.getJob(second.jobId!)?.resultText).toContain(`full-output="${secondUrl}"`);
	});

	it("keeps a useful 6000-character preview when the only early newline follows an opening brace", async () => {
		const output = `{\n${"x".repeat(7000)}`;
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			AgentRegistry.global().register({
				id: options.id,
			...
			return makeResult(options.id, { output });
		});
		...
		const text = manager.getJob(jobId)?.resultText ?? "";
		const response = text.match(/<response truncated="true">\n([\s\S]*?)\n<\/response>/)?.[1] ?? "";
		expect(response.startsWith("{\n")).toBe(true);
		expect(response.length).toBeGreaterThanOrEqual(5900);
		expect(response.length).toBeLessThanOrEqual(6000);
	});

	it("passes all repository context files, including AGENTS.md, into the worker", async () => {
		const contextFiles = [
			{ path: "/repo/AGENTS.md", content: "Repository rules", depth: 0 },
			{ path: "/repo/CLAUDE.md", content: "Additional rules", depth: 0 },
		];
		let receivedContextFiles: ToolSession["contextFiles"];
		let receivedMaxRequests: number | undefined;
		let receivedMaxRuntimeMs: number | undefined;
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			receivedContextFiles = options.contextFiles;
			receivedMaxRequests = options.maxRequests;
		...
		const manager = createManager();
		const session = { ...createSession({ manager }), contextFiles };
		const { jobId } = await VibeSessionRegistry.global().spawn(session, {
			cli: "good",
			name: "ContextWorker",
		...
		await manager.getJob(jobId)!.promise;
		expect(receivedContextFiles).toEqual(contextFiles);
		expect(receivedMaxRequests).toBe(200);
		expect(receivedMaxRuntimeMs).toBe(1_200_000);
	});

	it("applies tier defaults and preserves explicit per-turn limits on follow-ups", async () => {
		const firstLimits: Array<number | undefined> = [];
		const followUpLimits: Array<number | undefined> = [];
		vi.spyOn(executorModule, "runSubprocess").mockImplementation(async options => {
			firstLimits.push(options.maxRequests, options.maxRuntimeMs);
			AgentRegistry.global().register({
				id: options.id,
				displayName: options.id,
				kind: "sub",
		...
		vi.spyOn(executorModule, "runSubagentFollowUpTurn").mockImplementation(async options => {
			followUpLimits.push(options.maxRequests, options.maxRuntimeMs);
			return makeResult(options.id);
		});
		...
		const session = createSession({ manager });
		const registry = VibeSessionRegistry.global();
		const defaultFast = await registry.spawn(session, {
			cli: "fast",
			name: "DefaultFast",
			prompt: "Default limits.",
		});
		await manager.getJob(defaultFast.jobId)!.promise;
		...
		await manager.getJob(first.jobId)!.promise;
		const second = await registry.send(session, { session: "BoundedWorker", message: "Second." });
		await manager.getJob(second.jobId!)!.promise;
		expect(followUpLimits).toEqual([12, 3500]);
		expect(firstLimits).toEqual([100, 1_200_000, 12, 3500]);

		await expect(
			registry.spawn(session, { cli: "fast", name: "InvalidBudget", prompt: "No.", maxRequests: 0 }),
		).rejects.toThrow("maxRequests must be an integer between 1 and 1000");
		await expect(
			registry.spawn(session, { cli: "good", name: "InvalidTimeout", prompt: "No.", timeout: 100_000 }),
		).rejects.toThrow("timeout must be greater than 0 and at most 86400 seconds");
	});
	...
	it("send steers a streaming mid-turn worker and queues for a non-steerable one", async () => {
		...
		const queuedLater = await registry.send(session, { session: "Good", message: "Finally run checks." });
		expect(queued.mode).toBe("queued");
		expect(queuedLater.mode).toBe("queued");
		expect(registry.screens(session)[0]?.queued).toBe(2);
	...
	it("wait wakes on the first settled turn while the next shared-workspace turn remains in flight", async () => {
		...
		await pollUntil(() => gates.has("Fast"));
		expect(gates.has("Good")).toBe(false);
		expect(manager.getJob(good.jobId)?.queued).toBe(true);
		...
		await pollUntil(() => gates.has("Good"));
	...
	it("killAll terminates every session for the owner (mode-exit path)", async () => {
		...
		await pollUntil(() => gates.has("One"));
		expect(gates.has("Two")).toBe(false);
