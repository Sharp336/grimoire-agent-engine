...
describe("DesktopConfigAuthority", () => {
	...
	test("validates global writes, reset, scope boundaries, and revision conflicts", async () => {
		...
		await config.settingsWrite({ path: "compaction.enabled", value: true, scope: "global" }, revision);
		expect(settings.get("compaction.enabled")).toBe(true);
		await expect(
			config.settingsWrite({ path: "compaction.enabled", value: false, scope: "session" }),
		).rejects.toThrow("unsupported settings scope: session");
		await config.settingsWrite({ path: "compaction.enabled", reset: true, scope: "global" });
	...
	test("catalog is app-wire valid, sorted, and reports unavailable providers", async () => {
		...
		for (const setting of frame.items.filter(item => item.kind === "setting"))
			expect(setting.metadata?.scopes).toEqual(["global"]);
