...
describe("/session tag subcommands", () => {
	it("tag then tags reports the tag; untag then tags no longer reports it", async () => {
		...
		expect(await executeAcpBuiltinSlashCommand("/session tag foo", harness.runtime)).toEqual({
			consumed: true,
		});
		expect(harness.output).toHaveBeenCalledWith("Tagged session: foo");

		harness.output.mockClear();
		expect(await executeAcpBuiltinSlashCommand("/session tags", harness.runtime)).toEqual({
			consumed: true,
		});
		expect(harness.output).toHaveBeenCalledWith("Tags: foo");
		expect(sessionManager.sessionTags()).toEqual(["foo"]);

		harness.output.mockClear();
		expect(await executeAcpBuiltinSlashCommand("/session untag foo", harness.runtime)).toEqual({
			consumed: true,
		});
	...
	it("tag with no name is consumed and reports usage without appending an empty tag", async () => {
		...
		expect(await executeAcpBuiltinSlashCommand("/session tag", harness.runtime)).toEqual({
			consumed: true,
		});
...
describe("/session search", () => {
	...
	it("with no question reports usage and does not create the transcript database", async () => {
		...
		const acp = createAcpRuntime();
		expect(await executeAcpBuiltinSlashCommand("/session search", acp.runtime)).toEqual({
			consumed: true,
		});
