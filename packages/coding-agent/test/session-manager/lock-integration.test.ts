...
describe("SessionManager persistent lock integration", () => {
	...
	it("resolves and opens a session whose unchanged malformed lock is old enough for safe recovery", async () => {
		const { cwd, sessions } = fixture();
		fs.mkdirSync(cwd);
		...
		const resolved = await resolveResumableSession(path.basename(sessionFile, ".jsonl"), cwd, sessions);
		expect(resolved?.session.path).toBe(sessionFile);
		const resumed = await SessionManager.open(resolved!.session.path);
