...
describe("resolveEditMode", () => {
	...
	test("falls back from hashline to replace for Kiro models", () => {
		expect(resolveEditMode(createSession({ activeModel: "kiro/gpt-5.6-luna" }))).toBe("replace");
		expect(resolveEditMode(createSession({ activeModel: "kiro/gpt-5.6-sol" }))).toBe("replace");
	});
