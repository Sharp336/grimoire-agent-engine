...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".oh-omp");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".oh-omp"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".oh-omp", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".oh-omp", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".oh-omp", "SYSTEM.md"), "Project SYSTEM prompt");
