...
describe("resolveModelRoleValue", () => {
	...
	test("does not use secondopinion as a generic fallback role", () => {
		const settings = {
			getModelRole: (role: string) => (role === "secondopinion" ? "openai/gpt-4o" : undefined),
		} as Settings;

		const result = resolveModelFromSettings({ settings, availableModels: allModels });
