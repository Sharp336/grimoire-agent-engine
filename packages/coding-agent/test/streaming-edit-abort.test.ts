...
async function createLargeTargetSetup(abortCalls: { count: number }, onAbort?: () => void) {
...
it(
	...
	async () => {
		...
		const { target, removedLines, guard, makeEvent } = await createLargeTargetSetup(abortCalls, aborted.resolve);
		...
		try {
			...
			// setImmediate tick counts do not drain the guard's Bun.sleep(0) timers.
			// Await the observable abort; the beacon above already proved responsiveness.
			await withTimeout(aborted.promise, 5_000, "Missing-line scan did not abort the edit");
