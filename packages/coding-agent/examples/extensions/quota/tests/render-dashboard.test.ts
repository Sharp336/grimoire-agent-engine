...
describe("renderDashboard", () => {
	it("renders full dashboard with header, attention section, providers and footer", () => {
		const reports = [
			report({
				provider: "anthropic",
				metadata: { email: "kareem.jalal@jethur.com", orgName: "kareem.jalal@jethur.com's Organization" },
				limits: [
					limit({
						id: "5h",
						label: "5 Hour",
		...
		const model = buildQuotaDashboardModel(
			reports,
			NOW,
			new Map([["anthropic", { email: "kareem.jalal@jethur.com" }]]),
		);
		...
		expect(fullText).toContain("Anthropic · kareem.jalal@jethur.com · 7 Day");
		expect(fullText).toContain("Google Antigravity · osama15@gmail.com · Google · Weekly");
		...
		expect(fullText).not.toContain("kareem.jalal@jethur.com's Organization");
