...
async function rootFixture() {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-secure-fs-test-"));
	roots.push(root);
	return root;
}
...
describe("secure file jail native API", () => {
	it("round-trips binary bytes and revision guarded replacement", async () => {
		...
		expect((await fs.readFile(path.join(root, "nested.bin"))).equals(Buffer.from([9]))).toBe(true);
	});
	...
	it("lists sorted entries and enforces the iteration cap", async () => {
		const root = await rootFixture();
		await fs.writeFile(path.join(root, "z"), "z");
	...
	it("rejects final and parent symlinks", async () => {
		...
		const root = await rootFixture();
		const outside = await fs.mkdtemp(path.join(os.tmpdir(), "pi-secure-fs-outside-"));
		try {
			await fs.writeFile(path.join(outside, "secret"), "secret");
			await fs.symlink(path.join(outside, "secret"), path.join(root, "link"));
			await fs.symlink(outside, path.join(root, "dir-link"));
