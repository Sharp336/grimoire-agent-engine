...
- Added the `outlineCode` native binding (backed by `pi_ast::symbols::outline_code`) — a flat, pre-order tree-sitter symbol outline (name, kind, line ranges, container, depth/parent index) for 44 languages (the programming-language set, the GraphQL / Protobuf / SQL / Verilog / HCL / Nix / TLA+ DSLs, and the Dockerfile / CMake / Make / Just build files), powering the coding-agent `symbol` tool. Only data, markup, and component-wrapper formats (JSON, YAML, CSS, HTML, Markdown, Astro, Svelte, Vue, …) are excluded — they carry no addressable code symbols — and a Rust exhaustiveness test locks every `SupportLang` into exactly one of the supported/excluded buckets. Also added the `outlineLanguages` (with file extensions + fixed-name files), `isOutlineSupportedPath`, and `isOutlineSupportedLang` bindings so the TS layer derives its supported-file set, per-file gate, and explicit-`lang`-override validation (through the native `from_alias` table — canonical names AND aliases) from the native extractor (single source of truth, no drift).
