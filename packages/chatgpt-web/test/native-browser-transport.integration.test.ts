...
const chromiumProvisioned =
	Boolean(process.env.CHATGPT_WEB_TEST_CHROMIUM) && Boolean(process.env.CHATGPT_WEB_TEST_CHROMIUM_VERSION);
...
test.skipIf(!chromiumProvisioned)(
	"native inherited remote-debugging pipe connects to pinned Chromium",
	async () => {
		const executablePath = process.env.CHATGPT_WEB_TEST_CHROMIUM!;
		const version = process.env.CHATGPT_WEB_TEST_CHROMIUM_VERSION!;
		// Dynamic loading keeps the platform addon unloaded when the provisioned integration gate is skipped.
