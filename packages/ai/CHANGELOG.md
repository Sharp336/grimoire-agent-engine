...
- Added `requestyProvider` and `loginRequesty` — Requesty (`router.requesty.ai`) OpenAI-compatible provider with `validation: null` (public models endpoint cannot validate keys; bad keys surface on first real request)
