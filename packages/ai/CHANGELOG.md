...
- Fixed definitive OAuth refresh failures looping indefinitely when a byte-exact SQLite disable CAS missed a semantically unchanged credential row. Credential replay now requires a full persisted OAuth value change and is bounded to one peer-rotation replay.
