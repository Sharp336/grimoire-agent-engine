...
- Added Volcengine Agent Plan (火山引擎) as a new upstream provider (`volcengine-agent-plan`) with a dedicated base URL (`https://ark.cn-beijing.volces.com/api/plan`) and separate API key (`VOLCENGINE_AGENT_PLAN_API_KEY`), using the same model catalog as the Coding Plan with `ark-code-latest` as default.
- Added Volcengine Ark (火山引擎方舟) as a new pay-as-you-go provider (`volcengine`) using OpenAI-compatible API at `https://ark.cn-beijing.volces.com/api/v3` with `ARK_API_KEY` and dynamic model discovery, defaulting to `doubao-seed-2-1-pro-260628`.
