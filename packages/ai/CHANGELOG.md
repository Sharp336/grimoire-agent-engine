...
- OAuth providers can now declare `xaiHttpCompat: true` (and optionally `xaiHttpBaseUrl`) to indicate their credentials authenticate xAI HTTP tool endpoints (image gen, video gen, TTS, web search). Projected through `getOAuthProviders()` so plugin-registered providers can slot into xAI tool credential resolution without core naming them.
