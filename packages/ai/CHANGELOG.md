...
- Added `LITELLM_BASE_URL`, `LITELLM_API_KEY`, `VLLM_BASE_URL`, and `VLLM_API_KEY` environment variable support for local provider configuration, and brought LM Studio in line with `LM_STUDIO_BASE_URL`/`LM_STUDIO_API_KEY` env var fallbacks in the model manager.
