...
- Fixed Cursor hosted image generation (`GenerateImage` / Imagine) failing with `Workspace folder not found` on new conversations: `requestContext` now sends `env.workspace_paths` / `project_folder` plus `mcp_file_system_options.workspace_project_dir` from the session cwd, and new conversation state includes `previous_workspace_uris` as `file://` URIs. This is independent of grok-build's local `image_gen` tool.
- Fixed Cursor hosted image generation writing 0-byte files: proto3 `WriteArgs.file_text` decodes as `""` when unset, so `fileText ?? decode(fileBytes)` dropped the PNG in `file_bytes`. Non-empty `file_bytes` is now written raw (no UTF-8 / newline normalization), matching the proto contract used by GenerateImage / Imagine.
- Fixed Cursor hosted image generation writing 0-byte files: proto3 `WriteArgs.file_text` decodes as `""` when unset, so `fileText ?? decode(fileBytes)` dropped the PNG in `file_bytes`. Non-empty `file_bytes` is now written raw (no UTF-8 / newline normalization), and JSON/JS bridges that deliver `file_bytes` as base64, a Node Buffer, a number array, or `{ type: "Buffer", data }` (none of which have a usable `byteLength`) are decoded before the empty-text fallback. `encoding_hint=base64` on `file_text` is also treated as raw bytes. Matches the proto contract used by GenerateImage / Imagine.
- Fixed Cursor hosted image generation writing 0-byte files. The PNG lives on `generate_image_tool_call` as `GenerateImageSuccess.image_data` (base64) plus `file_path`; that conversation step was ignored, then a follow-up `writeArgs` with empty `file_text` / `file_bytes` created a successful empty file. Hosted Imagine results are now written to `file_path` (workspace-confined), empty image `WriteArgs` are rejected instead of executed, and `file_bytes` is still decoded from Uint8Array / JSON / base64 / Buffer / `$unknown` field 5.
- Fixed Cursor hosted image generation writing 0-byte files. The PNG lives on `generate_image_tool_call` as `GenerateImageSuccess.image_data` (base64) plus `file_path`; that conversation step was ignored, then a follow-up `writeArgs` with empty `file_text` / `file_bytes` created a successful empty file. Hosted Imagine results are now written to `file_path` (workspace-confined), empty image `WriteArgs` are rejected instead of executed, and `file_bytes` is still decoded from Uint8Array / JSON / base64 / Buffer / `$unknown` field 5. Paths under `env.project_folder` (`~/.cursor/projects/<slug>/`) are remapped onto the session cwd so Imagine PNGs land in the repo, not the Cursor artifact dir.
- Fixed Cursor hosted image generation (`GenerateImage` / Imagine) failing with `Workspace folder not found` on new conversations: `requestContext` now sends `env.workspace_paths` / `env.project_folder` plus `mcp_file_system_options.workspace_project_dir` from the session cwd (forwarded through the `cursor-agent` option mapper), and conversation state seeds `previous_workspace_uris` as `file://` URIs when the checkpoint list is empty, appending the live root if the session cwd later changes. `buildCursorRequestContext` takes an explicit `rules` list so a parallel `requestContext.rules` fill is not wiped. This is independent of grok-build's local `image_gen` tool.
