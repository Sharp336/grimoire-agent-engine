...
- Fixed Cursor MCP tool calls executing twice: mounted `xd://` devices and external MCP tools advertised into the Cursor request context are run once through the exec-channel bridge (which buffers the `toolResult` out-of-band), but the finalized `toolCall` block was left unstamped, so the shared agent-loop dispatch pass re-ran the same side-effecting call after `message_end`. Completed Cursor MCP blocks are now stamped with `kCursorExecResolved` when an MCP exec bridge is wired, mirroring the native exec path, so agent-loop skips them; without a bridge the block stays runnable so agent-loop still dispatches the tool locally ([#5650](https://github.com/can1357/oh-my-pi/issues/5650)).
