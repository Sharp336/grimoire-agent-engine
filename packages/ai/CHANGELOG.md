...
- Fixed legacy TypeBox `Type.Unsafe` schemas being re-closed on the model-facing wire: `toolWireSchema` now honors the legacy JSON Schema document marker, passing the authored document through instead of applying the Ark-derived `additionalProperties: false` closing policy ([#7068](https://github.com/can1357/oh-my-pi/issues/7068)).
