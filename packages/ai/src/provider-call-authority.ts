...
const CREDENTIAL_KEYS: Record<string, true> = {
	authorization: true,
...
const CREDENTIAL_HEADER_KEYS: Record<string, true> = {
	authorization: true,
...
function assertCredentialFree(value: unknown, seen = new Set<object>()): void {
	if (value === null || typeof value !== "object") return;
	if (seen.has(value)) throw new Error("Provider-call payload must be acyclic");
	seen.add(value);
	if (Array.isArray(value)) {
		for (const item of value) assertCredentialFree(item, seen);
	} else {
		for (const [key, child] of Object.entries(value)) {
			if (CREDENTIAL_KEYS[key.toLowerCase().replaceAll("-", "_")]) {
				throw new Error(`Provider-call payload contains forbidden credential field: ${key}`);
			}
...
export function validateProviderCallContext(model: Model<Api>, context: ProviderCallContext): void {
	if (context.mode !== "strict") throw new Error("Unsupported provider-call authority mode");
	...
	if (!expectedApiFamily || context.apiFamily !== expectedApiFamily) {
		throw new Error(`Provider-call authority does not support API family ${model.api}`);
	}
	const assignment = validateProviderCallOriginAssignment(context.originAssignment);
	const originEntry = resolveProviderCallOriginBinding(assignment.config_id, assignment.route_ordinal);
	if (
		context.configId !== assignment.config_id ||
		context.credentialGeneration !== assignment.credential_generation ||
		originEntry.provider !== context.provider ||
		originEntry.modelId !== context.modelId ||
		originEntry.apiFamily !== context.apiFamily
	) {
		throw new Error(`Provider-call origin assignment identity mismatch for ${context.configId}`);
	}
	if (!/^[1-9][0-9]*$/.test(context.callSequence)) {
		throw new Error("Provider-call authority callSequence must be a positive canonical decimal string");
	}
	for (const [name, value] of Object.entries({
		...
		if (!value.trim()) throw new Error(`Provider-call authority ${name} is required`);
	}
	...
	for (const value of [
		...
		if (!uuid.test(value)) throw new Error("Provider-call authority UUID fields must be lowercase canonical UUIDs");
	}
	if (
		!/^sha256:[0-9a-f]{64}$/.test(context.assignmentSha256) ||
		!/^sha256:[0-9a-f]{64}$/.test(context.tokenizerContractSha256)
	) {
	...
	if (context.expectedDimensions.length === 0) throw new Error("Provider-call authority dimensions are required");
	...
	for (const dimension of context.expectedDimensions) {
		...
		if (
			...
			throw new Error("Provider-call authority dimension vector is invalid or unsorted");
		}
		previousKey = key;
	}
}

interface ResponseDigest {
	hash?: string;
	bytes: number;
	ambiguous: boolean;
...
type SseTerminalProtocol = "none" | "openai-completions" | "openai-responses";

function isTerminalSseFrame(frame: string, protocol: SseTerminalProtocol): boolean {
	...
	if (protocol === "openai-completions") return payload === "[DONE]";
	try {
		const decoded = JSON.parse(payload) as { type?: unknown };
		const type = typeof decoded.type === "string" ? decoded.type : eventName;
...
function digestResponse(
	response: Response,
	protocol: SseTerminalProtocol,
	onResult: (result: ResponseDigest) => void,
): Response {
	const body = response.clone().body;
	if (!body) {
		onResult({
			hash: sha256(new Uint8Array()),
			bytes: 0,
			ambiguous: protocol !== "none",
	...
	void (async () => {
		...
		let decoder = protocol === "none" ? undefined : new TextDecoder("utf-8", { fatal: true });
		let frameBuffer = "";
		let bytes = 0;
		let sawTerminalFrame = protocol === "none";
		...
		try {
			while (true) {
				...
				if (decoder) {
					...
					if (decoded !== undefined && !sawTerminalFrame) {
						frameBuffer += decoded;
						while (true) {
							const boundary = /\r?\n\r?\n/.exec(frameBuffer);
							...
							if (isTerminalSseFrame(frame, protocol)) {
			...
			onResult({
				...
				bytes,
				ambiguous: protocolInvalid || !sawTerminalFrame,
				protocolInvalid,
				drained: true,
			});
		} catch {
			onResult({ hash: hash.digest("hex"), bytes, ambiguous: true, protocolInvalid, drained: false });
		}
	})();
	return response;
}
...
): ProviderCallDimension[] | undefined {
	const providerWindows = new Map(
		(message?.providerWindowDimensions ?? []).map(dimension => [
			`${dimension.dimension}\0${dimension.windowId}`,
			dimension,
		]),
	);
	const actual: ProviderCallDimension[] = [];
	for (const dimension of context.expectedDimensions) {
		let amount: string;
		switch (dimension.dimension) {
			...
			case "rpm_requests":
				amount = "1";
				break;
			case "tpm_input_tokens":
				if (message?.providerUsageReported !== true) return undefined;
				amount = String(message.usage.input + message.usage.cacheRead + message.usage.cacheWrite);
				break;
			case "tpm_output_tokens":
				if (message?.providerUsageReported !== true) return undefined;
				amount = String(message.usage.output);
				break;
			case "tpm_total_tokens":
				if (message?.providerUsageReported !== true) return undefined;
				amount = String(message.usage.totalTokens);
				break;
			case "provider_window": {
				const reported = providerWindows.get(`${dimension.dimension}\0${dimension.windowId}`);
				if (
					!reported ||
					reported.unitScale !== dimension.unitScale ||
					reported.windowStart !== dimension.windowStart ||
					reported.windowEnd !== dimension.windowEnd ||
					!/^(0|[1-9][0-9]*)$/.test(reported.amount)
				) {
					return undefined;
				}
				actual.push({ ...dimension, amount: reported.amount });
				continue;
			}
		}
		actual.push({ ...dimension, amount });
	}
	return actual;
}
...
function classifyFailure(
	...
	const quotaEvidence = `${providerErrorCode ?? ""} ${message?.errorMessage ?? ""}`;
	if (
		status === "429" &&
		/(?:insufficient[_ -]?quota|quota[_ -]?exhausted|billing[_ -]?hard[_ -]?limit)/i.test(quotaEvidence)
	) {
	...
	if (code !== undefined && code >= 300 && code < 400) return "http_3xx";
	if (code !== undefined && code >= 400 && code < 500) return "http_4xx";
	if (code !== undefined && code >= 500) return "http_5xx";
	if (message?.stopReason === "error") return "provider_protocol";
	return "none";
}
...
async function requestBodyBytes(input: string | URL | Request, init?: RequestInit): Promise<Uint8Array> {
	...
	throw new Error("Strict provider-call authority requires a deterministic byte request body");
}

function credentialFreeRequestMetadata(
	input: string | URL | Request,
	init?: RequestInit,
): { url: string; method: string; contentType: string; headers: Array<readonly [string, string]> } {
	const url = new URL(input instanceof Request ? input.url : input);
	for (const key of url.searchParams.keys()) {
		if (CREDENTIAL_QUERY_KEYS[key.toLowerCase()]) {
			throw new Error(`Provider-call URL contains forbidden credential query field: ${key}`);
		}
	}
	const method = (init?.method ?? (input instanceof Request ? input.method : "GET")).toUpperCase();
	const requestHeaders = new Headers(init?.headers ?? (input instanceof Request ? input.headers : undefined));
	const contentType = requestHeaders.get("content-type")?.trim() ?? "";
	const headers: Array<readonly [string, string]> = [];
	for (const [name, value] of requestHeaders) {
		const lowerName = name.toLowerCase();
		if (lowerName === "cookie" || lowerName === "set-cookie") {
			throw new Error("Provider-call request cookies are forbidden");
		}
		if (CREDENTIAL_HEADER_KEYS[lowerName]) {
			throw new Error(`Strict provider-call received credential-bearing RequestInit header: ${name}`);
		}
		headers.push([lowerName, value.trim()]);
	}
	...
	return { url: url.toString(), method, contentType, headers };
}

/** Owns the single-use issue permit and immutable receipt for one strict call. */
export class StrictProviderCallLifecycle {
	readonly #model: Model<Api>;
	readonly #context: ProviderCallContext;
	readonly #authority: ProviderCallAuthority;
	readonly #userOnPayload: SimpleStreamOptions["onPayload"];
	readonly #baseFetch: FetchImpl;
	readonly #credential: ProviderCallCredential;
	readonly #journal: ProviderCallJournal;
	...
	#providerErrorEvidence?: Promise<string | undefined>;
	#retryAfterAt?: string;
	#responseDigest?: ResponseDigest;
	#responseDigestPromise?: Promise<ResponseDigest>;
	...
	#receiptRecording?: Promise<void>;
	#fetchInvoked = false;

	constructor(model: Model<Api>, options: SimpleStreamOptions, authority: ProviderCallAuthority) {
		const context = options.providerCallContext;
		if (!context) throw new Error("Provider-call authority context is required");
		...
		const credential = options.providerCallCredential;
		if (!credential) throw new Error("Strict provider-call credential is required");
		if (credential.accountId !== context.accountId) {
			throw new Error("Provider-call credential account mismatch");
		}
		if (credential.credentialGeneration !== context.credentialGeneration) {
			throw new Error("Provider-call credential generation mismatch");
		}
		if (!credential.apiKey || !credential.bearerToken) {
			throw new Error("Strict provider-call credential material is empty");
		}
		const journal = options.providerCallJournal;
		if (!journal) throw new Error("Strict provider-call durable journal is required");
		this.#model = model;
	...
	options(options: SimpleStreamOptions): SimpleStreamOptions {
		const guardedFetch = Object.assign(this.#fetch.bind(this), { preconnect: this.#baseFetch.preconnect });
		return {
			...options,
			onPayload: this.#onPayload.bind(this),
			fetch: guardedFetch,
		};
	}
	...
	async #onPayload(payload: unknown, model?: Model<Api>): Promise<unknown> {
		...
		assertCredentialFree(shapedPayload);
		const canonicalPayload = canonicalProviderCallBytes(shapedPayload);
		if (this.#canonicalPayload && !Buffer.from(this.#canonicalPayload).equals(canonicalPayload)) {
			throw new Error("Provider-call issue permit rejects a changed retry payload");
		}
		this.#shapedPayload = shapedPayload;
		this.#canonicalPayload = canonicalPayload;
		return shapedPayload;
	}

	async #fetch(input: string | URL | Request, init?: RequestInit): Promise<Response> {
		if (this.#fetchInvoked) throw new Error("Provider-call issue permit is single-use; retry rejected");
		this.#fetchInvoked = true;
		if (!this.#canonicalPayload || this.#shapedPayload === undefined) {
			throw new Error("Provider-call authority requires final payload shaping before egress");
		}
		if (this.#permitConsumed || this.#reservation) {
			throw new Error("Provider-call issue permit is single-use; retry rejected");
		}
		const body = await requestBodyBytes(input, init);
		const metadata = credentialFreeRequestMetadata(input, init);
		assertProviderCallOrigin(
			this.#context.originAssignment,
		...
		if (metadata.contentType.includes("json")) {
			...
			try {
				serializedPayload = JSON.parse(new TextDecoder().decode(body));
			} catch {
				throw new Error("Provider-call JSON body is not valid UTF-8 JSON");
			}
			if (!Buffer.from(canonicalProviderCallBytes(serializedPayload)).equals(this.#canonicalPayload)) {
				throw new Error("Provider-call wire body differs from the exact post-shaping payload");
			}
		}
		const requestHashInput = {
			apiFamily: this.#context.apiFamily,
			httpMethod: metadata.method,
			credentialFreeUrl: metadata.url,
			contentType: metadata.contentType,
			headers: metadata.headers,
			body,
		};
		const canonicalRequest = encodeProviderCallRequestHashInput(requestHashInput);
		const requestSha256 = `sha256:${sha256(canonicalRequest)}`;
		const reserveRequest = {
			...
			contentType: metadata.contentType,
			headers: metadata.headers,
			payload: this.#shapedPayload,
			body,
			canonicalRequest,
			requestSha256,
			requestBodyBytes: String(body.byteLength),
		};
		const lease = await this.#journal.begin(this.#context, reserveRequest);
		this.#lease = lease;
		await this.#journal.markReserveSent(lease);
		const reservation = await this.#authority.reserve(reserveRequest);
		if (
			reservation.disposition !== "created" ||
			reservation.callSequence !== this.#context.callSequence ||
			reservation.idempotencyKey !== this.#context.idempotencyKey ||
			reservation.requestSha256 !== requestSha256 ||
			reservation.assignmentSha256 !== this.#context.assignmentSha256 ||
			!providerCallOriginAssignmentsEqual(reservation.originAssignment, this.#context.originAssignment) ||
			!reservation.reservationId ||
			!/^pcr1_[A-Za-z0-9_-]{43}$/.test(reservation.issuePermit)
		) {
			throw new Error("Provider-call authority returned no valid single-use issue permit");
		}
		const reservationReference: ProviderCallReservationReference = {
			reservationId: reservation.reservationId,
			disposition: reservation.disposition,
			callSequence: reservation.callSequence,
			idempotencyKey: reservation.idempotencyKey,
			requestSha256: reservation.requestSha256,
			issueAuthorizedAt: reservation.issueAuthorizedAt,
			assignmentSha256: reservation.assignmentSha256,
			originAssignment: reservation.originAssignment,
		};
		await this.#journal.storeReservation(lease, reservationReference);
		this.#reservation = reservationReference;
		this.#issuePermit = reservation.issuePermit;
		this.#permitConsumed = true;
		this.#requestBodyByteLength = body.byteLength;
		this.#attemptedAt = canonicalTimestamp();
		await this.#journal.storeProviderAttempt(lease, this.#attemptedAt);
		const providerHeaders = new Headers(init?.headers ?? (input instanceof Request ? input.headers : undefined));
		providerHeaders.set("Authorization", `Bearer ${this.#credential.bearerToken}`);
		try {
			const response = await this.#baseFetch(input, { ...init, headers: providerHeaders, redirect: "manual" });
			this.#requestBytesWritten = this.#requestBodyByteLength;
			this.#providerStatus = String(response.status);
			this.#requestId = response.headers.get("x-request-id") ?? response.headers.get("request-id") ?? undefined;
			this.#providerErrorCode =
				response.headers.get("x-provider-error-code") ??
				response.headers.get("x-error-code") ??
				response.headers.get("x-request-error-code") ??
				undefined;
			const retryAfter = response.headers.get("retry-after")?.trim();
			if (!this.#providerErrorCode && !response.ok && response.headers.get("content-type")?.includes("json")) {
				this.#providerErrorEvidence = readStrictAuthorityJson(response.clone(), "Provider error response")
					.then(value => {
			...
			if (retryAfter) {
				...
				if (Number.isFinite(target)) this.#retryAfterAt = `${new Date(target).toISOString().slice(0, -1)}000Z`;
			}
			const { promise, resolve } = Promise.withResolvers<ResponseDigest>();
			this.#responseDigestPromise = promise;
			const sse = response.ok && response.headers.get("content-type")?.includes("text/event-stream") === true;
			const protocol: SseTerminalProtocol = !sse
				? "none"
				: this.#model.api === "openai-completions"
					...
					: this.#model.api === "openai-responses" || this.#model.api === "azure-openai-responses"
						? "openai-responses"
						: "none";
			return digestResponse(response, protocol, result => {
				this.#responseDigest = result;
				resolve(result);
			});
		} catch (error) {
			await this.#recordReceipt("ambiguous", undefined, "connection_lost");
			throw error;
		}
	}
	...
	wrap(stream: AssistantMessageEventStream): AssistantMessageEventStream {
		...
		void (async () => {
			...
			try {
				...
				const { classification, ambiguityClass } = await this.#classifyTerminal(message);
				await this.#recordReceipt(classification, message, ambiguityClass);
				if (terminal) outer.push(terminal);
				else outer.end(await stream.result());
			} catch (error) {
				try {
					if (this.#reservation && !this.#receiptPrepared) {
						await this.#recordReceipt("ambiguous", undefined, "response_incomplete");
					}
				} catch (receiptError) {
					outer.fail(receiptError);
					return;
				}
				outer.fail(error);
			}
	...
	async #classifyTerminal(
		message: AssistantMessage | undefined,
	): Promise<{ classification: "terminal" | "ambiguous"; ambiguityClass?: ProviderCallAmbiguityClass }> {
		if (this.#responseDigestPromise) this.#responseDigest = await this.#responseDigestPromise;
		if (!this.#responseDigest) return { classification: "ambiguous", ambiguityClass: "connection_lost" };
		if (this.#responseDigest.protocolInvalid) {
			return { classification: "ambiguous", ambiguityClass: "response_incomplete" };
		}
		if (this.#responseDigest.ambiguous) return { classification: "ambiguous", ambiguityClass: "premature_eof" };
		if (
			message?.stopReason === "error" &&
			/(?:premature|incomplete|ended before|connection|reset|eof|truncated|socket)/i.test(message.errorMessage ?? "")
		) {
			return { classification: "ambiguous", ambiguityClass: "response_incomplete" };
		}
		if (!actualDimensions(this.#context, message)) {
			return { classification: "ambiguous", ambiguityClass: "usage_unknown" };
		}
	...
	async #recordReceipt(
		...
		if (this.#receiptRecorded || !this.#reservation || !this.#lease) return;
		if (this.#receiptRecording) return await this.#receiptRecording;
		this.#receiptRecording = (async () => {
			if (this.#responseDigestPromise) this.#responseDigest = await this.#responseDigestPromise;
			const providerFinishedAt = canonicalTimestamp();
			const bodyErrorCode = await this.#providerErrorEvidence;
			const inferredErrorCode =
				this.#providerErrorCode ??
				bodyErrorCode ??
				/(?:"code"|code)\s*[:=]\s*["']?([a-z0-9_.-]+)/i.exec(message?.errorMessage ?? "")?.[1];
			const receipt: ProviderCallReceiptRequest = {
				context: this.#context,
				reservation: this.#reservation!,
				receiptOperationId: this.#lease!.receiptOperationId,
				classification,
				...
				finalClassification: classification === "terminal" ? "TERMINAL_RESPONSE" : "AMBIGUOUS_ATTEMPT",
				drainState: this.#responseDigest?.drained ? "DRAINED" : "FROZEN",
				providerStartedAt: this.#attemptedAt,
				providerFinishedAt,
				httpStatus: this.#providerStatus,
				providerRequestId: this.#requestId,
				responseSha256: this.#responseDigest?.hash ? `sha256:${this.#responseDigest.hash}` : undefined,
				requestBytesWritten: String(this.#requestBytesWritten),
				responseBytesReceived: String(this.#responseDigest?.bytes ?? 0),
			};
			if (classification === "terminal") {
				receipt.failureClass = classifyFailure(this.#providerStatus, message, inferredErrorCode);
				receipt.providerErrorCode = inferredErrorCode;
				receipt.retryAfterAt = this.#retryAfterAt;
				receipt.actualDimensions = actualDimensions(this.#context, message);
			} else {
				receipt.ambiguityClass = ambiguityClass ?? "response_incomplete";
				receipt.requestMayHaveReachedProvider = this.#permitConsumed;
			}
			await this.#journal.storePendingReceipt(this.#lease!, receipt);
			this.#receiptPrepared = true;
			if (!this.#issuePermit) throw new Error("Provider-call receipt issue permit is unavailable");
			try {
				const acknowledgement = await this.#authority.recordReceipt(receipt, this.#issuePermit);
				await this.#journal.completeReceipt(this.#lease!, acknowledgement);
			} catch (error) {
				this.#issuePermit = undefined;
				throw error;
			}
			this.#issuePermit = undefined;
		...
		try {
			await this.#receiptRecording;
		} finally {
			this.#receiptRecording = undefined;
		}
	}
}
