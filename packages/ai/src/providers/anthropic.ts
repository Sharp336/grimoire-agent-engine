 ...
 */
const coworkHeaderKeys = new Set(Object.keys(coworkHeaders).map(key => key.toLowerCase()));
...
export function convertAnthropicMessages(
	...
	model: Model<"anthropic-messages">,
	isOAuthToken: boolean,
	opts?: { serverSideFallbackEnabled?: boolean; stripThinkingBlocks?: boolean },
		...
		if (msg.role === "user" || msg.role === "developer") {
			...
			for (const block of msg.content) {
				if (block.type === "text") {
					...
					if (opts?.stripThinkingBlocks) continue;
