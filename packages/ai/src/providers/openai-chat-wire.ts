...
export interface ChatCompletionChunkChoiceDelta {
	...
	/** The refusal message generated by the model. */
	refusal?: string | null;
	/** The role of the author of the chunk message. */
	role?: "developer" | "system" | "user" | "assistant" | "tool";
	tool_calls?: Array<ChatCompletionChunkChoiceDeltaToolCall>;
	/** Streaming audio output fragment (gpt-4o-audio with `modalities: ["audio"]`). */
	audio?: ChatCompletionChoiceDeltaAudio;
	/** Transcript of the streaming audio output fragment (some SDK variants). */
	audio_transcript?: string | null;
}

/** Streaming audio output fragment. `data` is a base64 slice; concatenate all
 * fragments across chunks and decode the whole at stream end (never per-fragment). */
export interface ChatCompletionChoiceDeltaAudio {
	id?: string;
	data?: string;
	expires_at?: number;
	transcript?: string;
