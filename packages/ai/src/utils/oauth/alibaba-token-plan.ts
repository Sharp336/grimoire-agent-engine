/**
 * Alibaba Token Plan Enterprise login flow.
 *
 * Provides OpenAI-compatible and Anthropic-compatible models via:
 *   https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1
 *   https://token-plan.ap-southeast-1.maas.aliyuncs.com/apps/anthropic
 *
 * This is not OAuth — it's a simple API key flow:
 * 1. Open browser to Alibaba Cloud Model Studio API key settings
 * 2. User copies their API key (format: sk-sp-...)
...
export async function loginAlibabaTokenPlan(options: OAuthController): Promise<string> {
	if (!options.onPrompt) {
		throw new Error("Alibaba Token Plan Enterprise login requires onPrompt callback");
	}
	...
	const apiKey = await options.onPrompt({
		message: "Paste your Alibaba Token Plan Enterprise API key",
		placeholder: "sk-sp-...",
	});
	...
	await validateOpenAICompatibleApiKey({
		provider: "Alibaba Token Plan Enterprise",
		apiKey: trimmed,
