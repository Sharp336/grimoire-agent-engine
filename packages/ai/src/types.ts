...
export interface StreamOptions {
	...
	/**
	 * Request that an audio-capable model (e.g. OpenAI `gpt-4o-audio-preview`)
	 * generate audio output in addition to text. Only honored by transports that
	 * serialize audio output (OpenAI chat completions). Ignored otherwise.
	 */
...
/**
 * Audio generated by a model (e.g. OpenAI `gpt-4o-audio-preview`). The `data`
 * is the full base64-encoded audio payload (wav/mp3) reassembled from the
 * streaming `delta.audio.data` fragments; decoded as a unit, never per-fragment.
 */
export interface AssistantAudioContent {
	type: "audio_output";
	data: string;
	mimeType: string;
...
export interface AudioOutputOptions {
	/** Voice preset understood by the provider (e.g. OpenAI: alloy/echo/...). */
	voice?: string;
	/** Output container format. Defaults to `"wav"`. */
	format?: "wav" | "mp3";
}
...
export interface AssistantMessage {
	role: "assistant";
	content: (TextContent | ThinkingContent | RedactedThinkingContent | AssistantAudioContent | ToolCall)[];
	api: Api;
	provider: Provider;
	model: string;
