...
function toolUseStartFrame(input = JSON.stringify({ path: "/etc/passwd" })): Uint8Array {
...
async function startServer(): Promise<string> {
	...
	server.on("stream", (stream: http2.ServerHttp2Stream) => {
		...
		if (scenario.kind === "invalid-tool-json") {
			stream.write(Buffer.concat([toolUseStartFrame('{"path":'), endTurnMetadataFrame()]));
			stream.end();
			return;
		}
		const partialFrame =
...
describe("Kiro terminal-state invariant", () => {
	...
	it("rejects invalid completed tool JSON even after END_TURN metadata", async () => {
		scenario = { kind: "invalid-tool-json" };
		const baseUrl = await startServer();
		const { eventTypes, result } = await collectStream(makeModel(baseUrl));

		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toContain("contained invalid JSON");
		expect(eventTypes[eventTypes.length - 1]).toBe("error");
		expect(eventTypes).not.toContain("thinking_end");
		expect(eventTypes).not.toContain("text_end");
		expect(eventTypes).not.toContain("toolcall_end");
