 ...
 * the `workos:` bearer prefix) was verified live against api.cline.bot and is
 * adapted from jellydn/pi-clinepass-provider (MIT).
 */
...
describe("readClineAuthCredentials", () => {
	it("reads WorkOS creds from the `cline` provider key", () => {
		const rec = readClineAuthCredentials(fileReader(providersJson("cline", 1_783_274_705_000)));
	...
	it("also reads the legacy `cline-pass` provider key", () => {
		const rec = readClineAuthCredentials(fileReader(providersJson("cline-pass", 1_783_274_705_000)));
		expect(rec?.accessToken).toBe("workos:eyJhbGciFAKE");
	});

	it("returns undefined when the credential file is absent", () => {
		expect(readClineAuthCredentials(fileReader(undefined))).toBeUndefined();
	});

	it("returns undefined when there is no auth block (static-key-only install)", () => {
		const json = JSON.stringify({ providers: { cline: { settings: { provider: "cline", apiKey: "sk_x" } } } });
		expect(readClineAuthCredentials(fileReader(json))).toBeUndefined();
	});
});
