...
async function requestJson(baseUrl: string, method: string, path: string, token: string | undefined, body?: unknown): Promise<Response> {
...
describe("auth-gateway management HTTP", () => {
	...
	test("requires an authenticated admin principal and rejects invalid management input", async () => {
		...
		expect(response.status).toBe(403);
		expect(await readJson(response)).toEqual({ error: { code: "forbidden", message: "Management routes require an admin token" } });

		response = await fetch(`${harness.handle.url}/v1/users`, { method: "POST", headers: jsonHeaders("legacy-token"), body: "{" });
		expect(response.status).toBe(400);
		expect(await readJson(response)).toEqual({ error: { code: "invalid_request", message: "Malformed JSON body" } });

		response = await requestJson(harness.handle.url, "POST", "/v1/users", "legacy-token", { name: "valid", unexpected: true });
		expect(response.status).toBe(400);
	});

	test("implements users, one-time tokens, ACLs, pool bindings, usage, and audit pagination", async () => {
		harness = await createGatewayHarness();
		let response = await requestJson(harness.handle.url, "POST", "/v1/users", "legacy-token", { name: "alice", description: "first", owner: "team-a" });
		...
		response = await requestJson(harness.handle.url, "PATCH", `/v1/users/${userId}`, "legacy-token", { enabled: false, role: "user", owner: null });
		expect(response.status).toBe(200);
		expect(expectObject(expectObject(await readJson(response)).user).enabled).toBe(false);
		response = await requestJson(harness.handle.url, "POST", `/v1/users/${userId}/tokens`, "legacy-token", { label: "replacement" });
		expect(response.status).toBe(201);
		body = expectObject(await readJson(response));
		...
		response = await requestJson(harness.handle.url, "POST", `/v1/users/${userId}/acl`, "legacy-token", { effect: "allow", kind: "route", pattern: "chat" });
		expect(response.status).toBe(201);
		body = expectObject(await readJson(response));
	...
	test("implements pool CRUD and validates live credential/provider pool members", async () => {
		...
		let response = await requestJson(harness.handle.url, "POST", "/v1/pools", "legacy-token", { name: "mainpool", provider: "mock", model: "model-a", strategy: "round-robin" });
		expect(response.status).toBe(201);
		let body = expectObject(await readJson(response));
		const poolId = Number(expectObject(body.pool).id);
		response = await requestJson(harness.handle.url, "POST", "/v1/pools", "legacy-token", { name: "mainpool", provider: "mock" });
		expect(response.status).toBe(409);
		...
		expect(response.status).toBe(200);
		response = await requestJson(harness.handle.url, "PATCH", `/v1/pools/${poolId}`, "legacy-token", { name: "renamed", strategy: "failover" });
		expect(response.status).toBe(200);

		response = await requestJson(harness.handle.url, "POST", `/v1/pools/${poolId}/members`, "legacy-token", { credentialId: credential.id });
		expect(response.status).toBe(201);
		response = await requestJson(harness.handle.url, "POST", `/v1/pools/${poolId}/members`, "legacy-token", { credentialId: credential.id });
		expect(response.status).toBe(200);
		response = await requestJson(harness.handle.url, "POST", `/v1/pools/${poolId}/members`, "legacy-token", { credentialId: 999999 });
		expect(response.status).toBe(400);
		response = await requestJson(harness.handle.url, "DELETE", `/v1/pools/${poolId}/members/${credential.id}`, "legacy-token");
		expect(response.status).toBe(204);

		const user = harness.accessStore.createUser({ name: "bindinguser" });
		response = await requestJson(harness.handle.url, "POST", `/v1/users/${user.user.id}/pools`, "legacy-token", { poolId });
		expect(response.status).toBe(201);
		response = await requestJson(harness.handle.url, "POST", `/v1/users/${user.user.id}/pools`, "legacy-token", { poolId });
		expect(response.status).toBe(200);
		response = await requestJson(harness.handle.url, "DELETE", `/v1/users/${user.user.id}/pools/${poolId}`, "legacy-token");
		expect(response.status).toBe(204);
