...
it("reuses enforced object schemas across shared branches", () => {
	...
	expect(secondaryItems).toBe(primaryItems);
});
...
it("keeps shared object schemas strict-compatible after adaptation", () => {
	...
	const opBranches = ((rootProperties.ops.items as Record<string, unknown>).anyOf ?? []) as Array<Record<string, unknown>>;
	const replaceTasks = ((opBranches[0]?.properties as Record<string, Record<string, unknown>>)?.tasks?.items ?? {}) as Record<string, unknown>;
	const updateTasks = ((((opBranches[1]?.properties as Record<string, Record<string, unknown>>)?.tasks?.anyOf ?? []) as Array<Record<string, unknown>>)[0]?.items ?? {}) as Record<string, unknown>;
	...
	expect(updateTasks).toBe(replaceTasks);
});
