...
function ProviderTotalsPanel({ providers }: { providers: ProviderAggregate[] }) {
	...
	const columns: DataTableColumn<ProviderAggregate>[] = [
		{ key: "provider", header: t("providers.totals.col.provider"), render: p => <span className="font-medium">{p.provider}</span> },
		{ key: "requests", header: t("providers.totals.col.requests"), numeric: true, render: p => formatInteger(p.totalRequests) },
		{
			key: "errors",
		...
		{ key: "cost", header: t("providers.totals.col.cost"), numeric: true, render: p => formatCost(p.totalCost, undefined, locale) },
		{ key: "tps", header: t("providers.totals.col.tps"), numeric: true, render: p => formatTokensPerSecond(p.avgTokensPerSecond) },
	];
...
function ProviderTrendPanel({ stats }: { stats: ProviderDashboardStats }) {
	...
	const formatValue = metric === "tokens" ? (v: number) => formatCompact(v, locale) : (v: number) => formatCost(v, undefined, locale);
...
function PeakHoursPanel({ hourly, providers }: { hourly: ProviderHourlyPoint[]; providers: ProviderAggregate[] }) {
	...
	const options = useMemo(() => {
		const { sharedScaleBase, yScale } = buildSharedScales({ chartTheme, formatY: (v: number) => formatCompact(v, locale) });
		return {
			responsive: true,
			maintainAspectRatio: false,
			plugins: buildSharedPlugins({
...
function WindowInsightsPanel({ insights }: { insights: ProviderWindowInsight[] }) {
	...
	const columns: DataTableColumn<ProviderWindowInsight>[] = [
		{ key: "provider", header: t("providers.windows.col.provider"), render: i => <span className="font-medium">{i.provider}</span> },
		{ key: "window", header: t("providers.windows.col.window"), render: i => i.windowLabel },
		{ key: "accounts", header: t("providers.windows.col.accounts"), numeric: true, render: i => formatInteger(i.accounts) },
		{
			key: "consumed",
			header: t("providers.windows.col.consumed"),
			numeric: true,
			render: i => (
				<span title={t("providers.windows.tooltip.consumed")}>
		...
		},
		{
			...
			numeric: true,
			render: i => (
				<span title={t("providers.windows.tooltip.peak")}>
	...
	return (
		<Panel
			...
			subtitle={t("providers.windows.subtitle")}
		>
			<DataTable
				columns={columns}
				data={insights}
