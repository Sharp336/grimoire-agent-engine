...
function metadata() {
  ...
  return {
    manifest: {
      schemaVersion: 1, appVersion: "17.2.5", platform: "win32", arch: "x64",
...
function authority(options = {}) {
  ...
  return { events, source, installed, native: {
    async openRuntimeBundle(request) { events.push("open"); if (options.openError) throw options.openError; assert.equal(request.expected.version, "17.2.5"); return source; },
...
test("runtime installation is native-authoritative, private, atomic, and versioned", async () => {
  ...
  assert.equal(result.bundle, fake.installed); assert.equal(result.versionKey, "17.2.5-win32-x64");
...
test("fixed metadata requires cli.js and mcp-main.js checksums and rejects traversal", () => {
  ...
  assert.throws(() => validateRuntimeBundleMetadata(value.manifest, value.checksums, { version: "17.2.5", platform: "win32", arch: "x64" }), /runtime_required_checksum_missing/);
  value.checksums.files["app/mcp-main.js"] = digest; value.manifest.entrypoints.cli = "../cli.js";
  assert.throws(() => validateRuntimeBundleMetadata(value.manifest, value.checksums, { version: "17.2.5", platform: "win32", arch: "x64" }), /runtime_fixed_entrypoints_invalid/);
});
