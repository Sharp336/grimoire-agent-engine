...
test("launcher state persists only allowlisted non-secret settings atomically", () => {
  ...
  try {
    ...
    store.update({ language: "zh-CN", onboardingComplete: true, runtimeMode: "full", browserSmokePassed: true, browserSmokeVersion: "17.2.4" });
    assert.deepEqual(createStateStore(file).read(), { ...DEFAULT_STATE, language: "zh-CN", onboardingComplete: true, runtimeMode: "full", browserSmokePassed: true, browserSmokeVersion: "17.2.4" });
