...
function fixture(options = {}) {
  ...
  const commandFactory = async ({ epoch, spawn, environment }) => { const current = spawnNumber; assert.deepEqual(environment, { opaqueEnvironment: spawn.index }); events.push(`spawn-${current}:command`); return { launchSpec: Object.freeze({ opaqueLaunch: current, instanceNonce: spawn.instanceNonce }), version: "17.2.7", runtimeEpoch: epoch.runtimeEpoch, lifecycleGeneration: epoch.lifecycleGeneration, instanceNonce: spawn.instanceNonce, close() { events.push(`spawn-${current}:command-close`); } }; };
...
test("runtime command forwards only the native materialized environment and fixed identity", async () => {
  ...
  const command = await resolveRuntimeCommand({
    installedRuntime: { bundle: Object.freeze({ opaqueBundle: true }), version: "17.2.7" },
    mode: "full",
