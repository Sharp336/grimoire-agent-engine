...
// On a model with `compat.supportsReasoningEffort: false` (xai-oauth/grok-build-0.1),
// this produced "Compaction failed: Thinking effort high is not supported by
// xai-oauth/grok-build-0.1" — but the underlying defect (silent user-intent
...
function getGrokBuildModel(): Model {
	const model = getBundledModel("xai-oauth", "grok-build-0.1");
	if (!model) throw new Error("Expected built-in xai-oauth/grok-build-0.1 to exist");
	return model;
}
...
describe("compaction thinking-level resolution (regression)", () => {
	...
	test("ThinkingLevel.High on xai-oauth/grok-build-0.1 → reasoning=undefined (clamp)", async () => {
		const spy = vi
			.spyOn(ai, "completeSimple")
			.mockResolvedValue(createAssistantMessage([{ type: "text", text: "handoff" }]));
...
describe("compact() propagates thinkingLevel to all three summarizers (regression)", () => {
	...
	test("xai-oauth/grok-build-0.1 + ThinkingLevel.High → every fan-out call gets reasoning=undefined (clamp)", async () => {
		const spy = vi
			.spyOn(ai, "completeSimple")
			.mockResolvedValue(createAssistantMessage([{ type: "text", text: "summary" }]));
