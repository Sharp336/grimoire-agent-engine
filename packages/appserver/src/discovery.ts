...
import { boundSnapshotEntries, uniqueEntryId } from "./snapshot-limits";
import type { FileSystem, SessionDiscovery, SessionRecord } from "./types";
...
const realFs: DiscoveryFileSystem = {
	...
	stat: async path => await fs.promises.stat(path),
	readdir: async path =>
		(await fs.promises.readdir(path, { withFileTypes: true })).map(entry => nodePath.join(path, entry.name)),
	readFile: async path => await fs.promises.readFile(path),
...
export function stableProjectId(cwd: string): ProjectId {
	let canonical = nodePath.resolve(cwd);
	try {
...
function parseTranscript(input: string | Uint8Array, path: string, host: HostId): SessionRecord {
	...
	const cwd = nodePath.resolve(header.cwd);
...
export class FileSessionDiscovery implements SessionDiscovery {
	readonly #index = new Map<string, FileIndexEntry>();
	...
	async #isSessionArtifactDirectory(path: string): Promise<boolean> {
		try {
	...
	async #files(path: string, depth = 0): Promise<string[]> {
	...
	async list(): Promise<SessionRecord[]> {
		...
		for (const path of files) {
			let identity: string;
			try {
				identity = fs.realpathSync.native(path);
			} catch {
				identity = nodePath.resolve(path);
			}
