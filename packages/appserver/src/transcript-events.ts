...
import type { RpcSessionEventFrame, RpcSubagentEventFrame } from "../../coding-agent/src/modes/rpc/rpc-types";
import type { AgentSessionEvent } from "../../coding-agent/src/session/agent-session";
import { cleanText } from "./discovery";
...
export class TranscriptEventTranslator {
	...
	#extensionUi(frame: Record<string, unknown>): AppserverEvent[] {
	...
	#correlatedAssistant(frame: Record<string, unknown>): { active: ActiveAssistant; streamId?: string } | undefined {
	...
	#messageStart(frame: Record<string, unknown>): TranscriptEvent[] {
		const message = frame.message;
		const snapshot = assistantSnapshot(message, () => this.#activeAssistant?.last?.at ?? this.#nowIso());
		if (!snapshot) return [];
	...
	#messageUpdate(frame: Record<string, unknown>): TranscriptEvent[] {
		const message = frame.message;
		const snapshot = assistantSnapshot(message, () => this.#activeAssistant?.last?.at ?? this.#nowIso());
		if (!snapshot) return [];
		const correlated = this.#correlatedAssistant(frame);
	...
	#messageEnd(frame: Record<string, unknown>): TranscriptEvent[] {
		const message = frame.message;
		const snapshot = assistantSnapshot(message, () => this.#activeAssistant?.last?.at ?? this.#nowIso());
		if (!snapshot) return [];
		const correlated = this.#correlatedAssistant(frame);
	...
	#messagePersisted(frame: Record<string, unknown>): TranscriptEvent[] {
		const streamId = asString(frame.streamId);
	...
	#settleMessage(streamId: string, durableEntryId: string, at: string): TranscriptEvent[] {
	...
	#emitMessage(snapshot: MessageSnapshot): TranscriptEvent[] {
	...
	#toolStart(frame: Record<string, unknown>): TranscriptEvent[] {
	...
	#toolProgress(frame: Record<string, unknown>): TranscriptEvent[] {
		const callId = asString(frame.toolCallId);
	...
	#toolResult(frame: Record<string, unknown>): TranscriptEvent[] {
		const callId = asString(frame.toolCallId);
		const state = callId ? this.#toolStates.get(callId) : undefined;
