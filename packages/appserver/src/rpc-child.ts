...
import type { RpcResponse, RpcSessionEntryFrame } from "../../coding-agent/src/modes/rpc/rpc-types";
import type { ManagedRpcImageRef } from "./image-upload-store";
import type { ChildHandle, RpcChildFactory, SessionRecord } from "./types";
...
export function resolveRpcChildInvocation(overrides: RpcChildInvocationOverrides = {}): RpcChildInvocation {
	...
	const main = runningCodingAgentDaemon ? path.resolve(path.dirname(runningMain), "../cli.ts") : runningMain;
...
async function* lines(stream: AsyncIterable<string | Uint8Array>): AsyncGenerator<string> {
	...
	for await (const chunk of stream) {
		...
		while (index >= 0) {
			...
			if (stringBytes(line) > MAX_LINE_BYTES) throw new Error("rpc line exceeds 1MiB");
			yield line;
...
export class RpcChildSupervisor {
	...
	async #readStdout(ready: { resolve: () => void; reject: (error: Error) => void }): Promise<void> {
		try {
	...
	#terminateAfterReaderFailure(): void {
	...
	#fail(error: Error, terminateChild = false): void {
