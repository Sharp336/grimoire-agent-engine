...
test("rebinds a canonical cached transcript when its live symlink alias changes", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "omp-discovery-alias-"));
	const target = path.join(root, "session-target.jsonl");
	const firstAlias = path.join(root, "alias-a.jsonl");
	const secondAlias = path.join(root, "alias-b.jsonl");
	...
	try {
		...
		await fs.symlink(target, firstAlias);
		...
		await fs.rm(firstAlias);
		await fs.symlink(target, secondAlias);
		const warmAlias = await discovery.list();
		expect(warmAlias[0]?.path).toBe(secondAlias);
		await expect(fs.stat(warmAlias[0]!.path)).resolves.toBeDefined();
	} finally {
