...
import { Process as NativeProcess } from "@oh-my-pi/pi-natives";
import { rpcTransportFrame } from "../../coding-agent/src/modes/rpc/rpc-mode.ts";
...
describe("identity and socket ownership", () => {
	test("host identity persists while epochs are fresh", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-id-"));
		const path = nodePath.join(root, "host-id");
	...
	test("refuses regular path and removes stale socket", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-sock-"));
		const path = nodePath.join(root, "app.sock");
		await fs.writeFile(path, "regular");
	...
	test("recovers confirmed-dead owner residue with stale socket", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-stale-"));
		const path = nodePath.join(root, "app.sock");
		...
		await expect(fs.stat(`${path}.owner`)).rejects.toThrow();
	});
	test("recovers an old crash-truncated owner under a private rename claim", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-truncated-owner-"));
		const path = nodePath.join(root, "app.sock");
		await fs.writeFile(`${path}.owner`, '{"version":3,"ownerId":', { mode: 0o600 });
		const old = new Date(Date.now() - 10_000);
		await fs.utimes(`${path}.owner`, old, old);

		const appserver = createAppserver({ hostId: host, socketPath: path, discovery: new StaticDiscovery([]) });
		await appserver.start();
		await appserver.stop();
		expect((await fs.readdir(root)).filter(name => name.includes("owner-recovery"))).toEqual([]);
		await expect(fs.stat(`${path}.owner`)).rejects.toThrow();
	});
	test("fresh malformed owners fail closed", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-fresh-owner-"));
		const path = nodePath.join(root, "app.sock");
		await fs.writeFile(`${path}.owner`, "not-json", { mode: 0o600 });

		const appserver = createAppserver({ hostId: host, socketPath: path, discovery: new StaticDiscovery([]) });
		await expect(appserver.start()).rejects.toThrow("too fresh to recover");
		expect(await fs.stat(`${path}.owner`)).toBeDefined();
	});
	test("a reused live PID with a different process marker is stale", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-reused-pid-"));
		const path = nodePath.join(root, "app.sock");
		const ownerId = "44444444-4444-4444-8444-444444444444";
		const currentMarker = NativeProcess.fromPid(process.pid)?.startMarker;
		if (!currentMarker) throw new Error("missing native marker for current test process");
		const markerSource = currentMarker.slice(0, currentMarker.indexOf(":"));
		await fs.writeFile(
			`${path}.owner`,
			JSON.stringify({
				version: 3,
				ownerId,
				pid: process.pid,
				processStartMarker: `${markerSource}:older-process`,
				backingName: `.appserver-${ownerId}.sock`,
				device: 0,
				inode: 0,
			}),
	...
	test("active and concurrent owners are rejected", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-owner-"));
		const path = nodePath.join(root, "app.sock");
		...
		await expect(second.start()).rejects.toThrow("ownership guard is busy");
		await first.stop();
		expect(await unixSocketActive(path)).toBe(false);
		// The permanent inode remains while the native handle lock is released.
		expect((await fs.stat(guardPath)).mode & 0o777).toBe(0o600);
		await second.start();
		expect(await unixSocketActive(path)).toBe(true);
		await second.stop();
...
describe("child supervision", () => {
	...
	test("a large aggregate stdout chunk is split before per-line bounds are enforced", async () => {
		const gate = Promise.withResolvers<void>();
		const exited = Promise.withResolvers<number>();
		...
		const aggregate = `${JSON.stringify({ type: "ready" })}\n${frames.map(frame => JSON.stringify(frame)).join("\n")}\n`;
		expect(new TextEncoder().encode(aggregate).byteLength).toBeGreaterThan(1024 * 1024);
		for (const frame of frames) expect(() => parseBounded(JSON.stringify(frame))).not.toThrow();
		const child: ChildHandle = {
			stdin: { write: () => {} },
			stdout: (async function* () {
				yield aggregate;
		...
		const supervisor = new RpcChildSupervisor(
			...
			record("s"),
			{
				...
				event: frame => {
					events.push(frame);
					if (events.length === frames.length) allSeen.resolve();
				},
				crashed: error => crashed.push(error),
			},
		);
		...
		expect(events.map(frame => frame.id)).toEqual(frames.map(frame => frame.id));
		expect(crashed).toEqual([]);
		supervisor.stop();
	});
	test("projected image, MCP, and session frames complete without crashing the supervisor", async () => {
		const gate = Promise.withResolvers<void>();
		const exited = Promise.withResolvers<number>();
		const allSeen = Promise.withResolvers<void>();
		...
		const rawFrames = [
			...
			{
				type: "session_entry",
				entry: { type: "message", id: "image-entry", parentId: null, timestamp: stamp, message },
			},
			{
				type: "tool_execution_end",
				toolCallId: "mcp-call-large",
				toolName: "mcp__example__read",
				isError: false,
				result: {
					content: [{ type: "text", text: "bounded summary" }],
					details: { rawContent: [{ type: "text", text: "r".repeat(1_100_000) }] },
				},
			},
			{
				type: "session_entry",
				entry: {
					type: "custom",
					id: "large-text-entry",
					parentId: "image-entry",
					timestamp: stamp,
					customType: "mcp-result",
					data: { text: "s".repeat(1_100_000) },
				},
			},
			{ type: "agent_end", messages: [] },
		];
		const frames = rawFrames.map(frame => rpcTransportFrame(frame, true));
		expect(new TextEncoder().encode(JSON.stringify(rawFrames[0])).byteLength).toBeGreaterThan(1024 * 1024);
		for (const frame of frames) expect(() => parseBounded(JSON.stringify(frame))).not.toThrow();
		...
		const supervisor = new RpcChildSupervisor(
			...
			{
				entry: frame => {
					entries.push(frame as unknown as Record<string, unknown>);
					if (entries.length + events.length === rawFrames.length) allSeen.resolve();
				},
				event: frame => {
					events.push(frame);
					if (entries.length + events.length === rawFrames.length) allSeen.resolve();
		...
		expect(events.map(frame => frame.type)).toEqual(["message_start", "tool_execution_end", "agent_end"]);
		expect(events[1]?.toolCallId).toBe("mcp-call-large");
		expect(crashed).toEqual([]);
		supervisor.stop();
	});
	...
	test("real Unix WebSocket upgrades on the local socket", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-ws-"));
		const path = nodePath.join(root, "app.sock");
		const appserver = createAppserver({ hostId: host, socketPath: path, discovery: new StaticDiscovery([]) });
		await appserver.start();
		...
		const receivedHandshake = Promise.withResolvers<Buffer>();
		socket.once("data", receivedHandshake.resolve);
		const handshake = await receivedHandshake.promise;
		...
		const receivedFrame = Promise.withResolvers<Buffer>();
		socket.once("data", receivedFrame.resolve);
		const frame = await receivedFrame.promise;
...
describe("appserver startup and cleanup", () => {
	test("startup failure releases ownership and stop kills every child", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-clean-"));
		const path = nodePath.join(root, "app.sock");
		const factory = new IdleFactory();
		...
		const recovered = createAppserver({ hostId: host, socketPath: path, discovery: new StaticDiscovery([]) });
