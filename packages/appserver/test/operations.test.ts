...
describe("desktop operation dispatcher", () => {
	...
	test("flushes terminal output emitted synchronously during open after ownership is claimed", async () => {
			...
			authority({
				termOpen: async (_args, operationContext) => {
					operationContext.emitTerminalOutput?.({
						...
						hostId: context.hostId,
						sessionId: context.sessionId,
						terminalId: terminalId("term-initial"),
						cursor: { epoch: "initial", seq: 1 },
						stream: "stdout",
						data: "ready",
					});
					return { terminalId: "term-initial" };
				},
			}),
			new TerminalOwnerRegistry(),
			frame => outputs.push(frame),
		);

		await expect(dispatcher.dispatch(command("term.open"), context)).resolves.toMatchObject({
			terminalId: "term-initial",
		});
		expect(outputs).toHaveLength(1);
		expect(outputs[0]).toMatchObject({ type: "terminal.output", data: "ready" });
	});
	test("discards pending initial output when terminal ownership claim fails", async () => {
		...
		registry.claim({
			...
			hostId: context.hostId,
			sessionId: context.sessionId!,
			terminalId: terminalId("term-conflict"),
		});
		const outputs: unknown[] = [];
		let closes = 0;
		const dispatcher = new DesktopOperationDispatcher(
			authority({
				termOpen: async (_args, operationContext) => {
					...
					return { terminalId: "term-conflict" };
				},
				terminalClose: async () => {
					closes += 1;
				},
