...
import { appserverLockCheck, appserverLockReclaim } from "../../coding-agent/src/session/appserver-authority";
...
const RPC_LOCK_CHILD = nodePath.join(import.meta.dir, "fixtures", "rpc-lock-child.ts");
...
class GatedSnapshotDiscovery implements SessionDiscovery {
	...
	async list(): Promise<SessionRecord[]> {
		...
		if (!gate) return this.values;
		this.#nextGate = undefined;
		const snapshot = this.values.map(value => ({ ...value, entries: [...value.entries] }));
...
class FakeAuthority implements SessionAuthority {
	...
	failLifecycle = false;
	committedLifecycleFailure?: "archive" | "restore" | "delete";
	constructor(private readonly values: SessionRecord[] = []) {}
	#throwCommitted(operation: "archive" | "restore" | "delete"): void {
		if (this.committedLifecycleFailure !== operation) return;
		this.committedLifecycleFailure = undefined;
		throw Object.assign(new Error(`committed ${operation} release failure`), {
			committed: true,
			operation,
		});
	}
...
async function liveServer(
	...
	const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-appserver-live-"));
	const path = nodePath.join(root, "run", "app.sock");
...
describe("live Unix websocket protocol", () => {
	...
	test("reads only attached transcript-entry images in bounded uncached chunks", async () => {
		const blobRoot = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-appserver-blobs-"));
		...
		await fs.writeFile(nodePath.join(blobRoot, sha256), png, { mode: 0o600 });
		...
		const { appserver, path, root } = await liveServer(new LiveFactory(), [imageRecord], 256, undefined, blobRoot);
		try {
...
describe("raw RFC6455 boundary and lifecycle", () => {
	...
	test("stop after a spawned child closes clients and removes socket, owner, and transient files", async () => {
		...
		await appserver.stop();
		await client.client.closed();
		expect(await fs.stat(path).catch(() => undefined)).toBeUndefined();
		await factory.children[0]!.killed.promise;
		const files = await fs.readdir(nodePath.join(root, "run"));
		expect(
	...
	test("owner crash residue and concurrent stale recovery are deterministic", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-owner-live-"));
		const path = nodePath.join(root, "run", "app.sock");
		await fs.mkdir(nodePath.join(root, "run"), { recursive: true, mode: 0o700 });
		await fs.writeFile(`${path}.owner.tmp-orphan`, "orphan");
		...
		await first.start();
		await first.stop();
		expect((await fs.stat(`${path}.guard`)).mode & 0o777).toBe(0o600);
		...
		await fs.writeFile(
			...
			JSON.stringify({
				...
				ownerId: staleOwner,
				pid: 999999,
				processStartMarker: "dead:999999",
		...
		await b.stop();
		expect((await fs.stat(`${path}.guard`)).mode & 0o777).toBe(0o600);
		await fs.writeFile(`${path}.owner`, "not-json", { mode: 0o600 });
		...
		await cleanup.stop();
		expect(await fs.stat(`${path}.owner`)).toBeDefined();
	});
});
describe("WS command boundary, authority, confirmation, and lock lifecycle", () => {
	...
	test("confirmation challenge is one-shot, connection-bound, expiry-aware, and gated by revision", async () => {
		...
		const { appserver, path } = await liveServer(factory, values, 256, new FakeAuthority(values));
	...
	test("lifecycle mutations converge host-wide while events remain attached and archived sessions stay read-only", async () => {
		...
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("committed lifecycle failures reconcile archive, restore, and delete projection truth", async () => {
		const values = [record("s1")];
		const authority = new FakeAuthority(values);
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-committed-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
		...
		authority.committedLifecycleFailure = "archive";
		client.client.sendJson({
			...command("archive-committed", "archive-committed", "session.archive", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const archived = await untilResponse(client.client, "archive-committed");
		expect(archived.frames).toEqual([
			expect.objectContaining({
				type: "session.delta",
			...
			expect.objectContaining({
				type: "response",
				ok: false,
				error: expect.objectContaining({ code: "session_lifecycle_failed" }),
			}),
		]);
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toEqual(expect.any(String));

		authority.committedLifecycleFailure = "restore";
		client.client.sendJson({
			...command("restore-committed", "restore-committed", "session.restore", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const restored = await untilResponse(client.client, "restore-committed");
		expect(restored.response).toMatchObject({
			ok: false,
			error: { code: "session_lifecycle_failed" },
		});
		expect(
			restored.frames.some(frame => frame.type === "session.delta" && frame.upsert?.archivedAt === undefined),
		).toBe(true);
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toBeUndefined();

		authority.committedLifecycleFailure = "delete";
		client.client.sendJson({
			...command("delete-committed", "delete-committed", "session.delete", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const challenge = await client.client.nextServer();
		expect(challenge.type).toBe("confirmation");
		if (challenge.type !== "confirmation") throw new Error("missing committed delete confirmation");
		client.client.sendJson(
			confirmFrame(
				"delete-committed-confirm",
		...
		const deleted = await untilResponse(client.client, "delete-committed");
		expect(deleted.frames).toEqual([
			expect.objectContaining({ type: "session.delta", remove: "s1" }),
			expect.objectContaining({
				type: "response",
				ok: false,
				error: expect.objectContaining({ code: "session_lifecycle_failed" }),
	...
	test("older gated session list snapshots cannot overwrite committed lifecycle reconciliation", async () => {
		const values = [record("s1")];
		const discovery = new GatedSnapshotDiscovery(values);
		const authority = new FakeAuthority(values);
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-refresh-race-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
		...
		await appserver.start();
		const reader = await readyClient(appserver.socketPath, ["sessions.read"]);
		const manager = await readyClient(appserver.socketPath, ["sessions.read", "sessions.manage"]);

		const archiveGate = discovery.blockNext();
		reader.client.sendJson(hostCommand("list-before-archive", "list-before-archive", "session.list", {}));
		await archiveGate.started;
		authority.committedLifecycleFailure = "archive";
		manager.client.sendJson({
			...command("archive-race", "archive-race", "session.archive", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		expect((await untilResponse(manager.client, "archive-race")).response).toMatchObject({
			ok: false,
			error: { code: "session_lifecycle_failed" },
		});
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toEqual(expect.any(String));
		archiveGate.release();
		expect((await untilResponse(reader.client, "list-before-archive")).response).toMatchObject({
			ok: true,
			result: { sessions: [expect.objectContaining({ sessionId: "s1", archivedAt: expect.any(String) })] },
		});
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toEqual(expect.any(String));

		const restoreGate = discovery.blockNext();
		reader.client.sendJson(hostCommand("list-before-restore", "list-before-restore", "session.list", {}));
		await restoreGate.started;
		authority.committedLifecycleFailure = "restore";
		manager.client.sendJson({
			...command("restore-race", "restore-race", "session.restore", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		expect((await untilResponse(manager.client, "restore-race")).response).toMatchObject({
			ok: false,
			error: { code: "session_lifecycle_failed" },
		});
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toBeUndefined();
		restoreGate.release();
		expect((await untilResponse(reader.client, "list-before-restore")).response).toMatchObject({
			ok: true,
			result: { sessions: [expect.not.objectContaining({ archivedAt: expect.anything() })] },
		});
		expect(appserver.snapshot(sid("s1"))?.ref.archivedAt).toBeUndefined();

		const deleteGate = discovery.blockNext();
		reader.client.sendJson(hostCommand("list-before-delete", "list-before-delete", "session.list", {}));
		await deleteGate.started;
		authority.committedLifecycleFailure = "delete";
		manager.client.sendJson({
			...command("delete-race", "delete-race", "session.delete", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const challenge = await manager.client.nextServer();
		expect(challenge.type).toBe("confirmation");
		if (challenge.type !== "confirmation") throw new Error("missing refresh-race delete confirmation");
		manager.client.sendJson(
			confirmFrame("delete-race-confirm", String(challenge.confirmationId), "delete-race", "approve", "s1"),
		);
		expect((await untilResponse(manager.client, "delete-race")).response).toMatchObject({
			ok: false,
			error: { code: "session_lifecycle_failed" },
		});
		expect(appserver.snapshot(sid("s1"))).toBeUndefined();
		deleteGate.release();
		expect((await untilResponse(reader.client, "list-before-delete")).response).toMatchObject({
			ok: true,
			result: { sessions: [], totalCount: 0 },
		});
		expect(appserver.snapshot(sid("s1"))).toBeUndefined();

		await closeClients([reader.client, manager.client]);
		await appserver.stop();
	});
	...
	test("active deletion is refused before challenge and failed metadata mutation emits no lifecycle frames", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-guard-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("a stale scheduled state refresh cannot resurrect streaming after agent end", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-stale-state-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("agent end clears streaming state that a scheduled refresh already projected", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-terminal-state-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("queued state and a pending non-streaming RPC both fail lifecycle operations closed", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-pending-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("a deferred desktop write blocks archive until the write has fully settled", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-write-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("deleting the last session closes its open terminal before removing the projection", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-terminal-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("a hung terminal close times out and releases the lifecycle fence", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-terminal-timeout-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("lifecycle quiesce escalates a graceful-stop timeout before mutating metadata", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-supervisor-force-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("confirmed close reaps a forced busy child, settles working state, and permits archive", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory("forced");
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-close-supervisor-force-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("a throwing child kill rolls back explicit close and becomes restartable after exit", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-close-kill-throw-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("archive observes a real RPC child's lock release after lifecycle quiesce", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-real-lock-live-"));
		const sessionPath = nodePath.join(root, "session.jsonl");
		await fs.writeFile(sessionPath, `${JSON.stringify({ type: "session", id: "s1", cwd: root })}\n`);
	...
	test("forced SIGKILL close reclaims only the exited real child's lock for immediate archive and delete", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-real-kill-lock-live-"));
		const sessionPath = nodePath.join(root, "session.jsonl");
		await fs.writeFile(sessionPath, `${JSON.stringify({ type: "session", id: "s1", cwd: root })}\n`);
		const session = record("s1");
		session.path = sessionPath;
		...
		const factory = new BunRpcChildFactory({
			...
			prefixArgv: [RPC_LOCK_CHILD, "--ignore-sigterm"],
		});
		const appserver = createAppserver({
			hostId: host,
			epoch,
			...
			lifecycleQuiesceTimeoutMs: 25,
		});
		await appserver.start();
		const client = await readyClient(appserver.socketPath, ["sessions.read", "sessions.manage", "sessions.prompt"]);

		client.client.sendJson(command("forced-real-state", "forced-real-state", "session.state.get", "s1", {}));
		expect((await untilResponse(client.client, "forced-real-state")).response.ok).toBe(true);
		const child = appserver.childFor(sid("s1"));
		if (!child?.pid) throw new Error("real child pid is unavailable");
		const childPid = child.pid;
		expect(inspectSessionLock(sessionPath)).toMatchObject({ status: "live", processAlive: true });

		client.client.sendJson({
			...command("forced-real-close", "forced-real-close", "session.close", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const closeChallenge = await client.client.nextServer();
		if (closeChallenge.type !== "confirmation") throw new Error("missing close confirmation");
		client.client.sendJson(
			confirmFrame(
				"forced-real-close-confirm",
				String(closeChallenge.confirmationId),
				"forced-real-close",
				"approve",
				"s1",
			),
		);
		expect((await untilResponse(client.client, "forced-real-close")).response).toMatchObject({
			ok: true,
			result: { closed: true },
		});
		expect(inspectSessionLock(sessionPath).status).toBe("missing");
		...
		client.client.sendJson({
			...command("forced-real-archive", "forced-real-archive", "session.archive", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		expect((await untilResponse(client.client, "forced-real-archive")).response).toMatchObject({
			ok: true,
			result: { archived: true },
		});

		client.client.sendJson({
			...command("forced-real-delete", "forced-real-delete", "session.delete", "s1", {}),
			expectedRevision: appserver.snapshot(sid("s1"))!.revision,
		});
		const deleteChallenge = await client.client.nextServer();
		if (deleteChallenge.type !== "confirmation") throw new Error("missing delete confirmation");
		client.client.sendJson(
			confirmFrame(
				"forced-real-delete-confirm",
				String(deleteChallenge.confirmationId),
				"forced-real-delete",
				"approve",
				"s1",
			),
		);
		expect((await untilResponse(client.client, "forced-real-delete")).response).toMatchObject({
			ok: true,
			result: { deleted: true },
		});
		expect(authority.lifecycle).toEqual(["archive:s1", "delete:s1"]);

		await closeClients([client.client]);
		await appserver.stop();
	});

	test("daemon stop escalates a real child to SIGKILL and reclaims its dead-owner lock", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-stop-real-kill-lock-live-"));
		const sessionPath = nodePath.join(root, "session.jsonl");
		await fs.writeFile(sessionPath, `${JSON.stringify({ type: "session", id: "s1", cwd: root })}\n`);
		const session = record("s1");
		session.path = sessionPath;
		...
		const appserver = createAppserver({
			...
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
			sessionAuthority: new FakeAuthority([session]),
			childFactory: factory,
			lockCheck: appserverLockCheck,
			lockReclaim: appserverLockReclaim,
			lifecycleQuiesceTimeoutMs: 25,
		...
		await appserver.start();
		const client = await readyClient(appserver.socketPath, ["sessions.read", "sessions.manage", "sessions.prompt"]);
		client.client.sendJson(command("stop-real-state", "stop-real-state", "session.state.get", "s1", {}));
		expect((await untilResponse(client.client, "stop-real-state")).response.ok).toBe(true);
		const child = appserver.childFor(sid("s1"));
		if (!child?.pid) throw new Error("real child pid is unavailable");
		const childPid = child.pid;
		expect(inspectSessionLock(sessionPath)).toMatchObject({ status: "live", processAlive: true });
	...
	test("a real RPC child releases its session lock when stdin reaches EOF", async () => {
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-rpc-eof-lock-live-"));
		const sessionPath = nodePath.join(root, "session.jsonl");
		await fs.writeFile(sessionPath, `${JSON.stringify({ type: "session", id: "s1", cwd: root })}\n`);
	...
	test("a child that survives forced termination stays owned and blocks lifecycle retries", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-supervisor-stuck-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("an opposite lifecycle request cannot report a no-op success while archive is in flight", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-lifecycle-opposite-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("confirmed close fences a second close and archive while quiescing the runtime", async () => {
		...
		const authority = new FakeAuthority(values);
		const factory = new LiveFactory("manual");
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-close-lifecycle-fence-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("session list reconciliation broadcasts newly discovered safe metadata", async () => {
		...
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-reconcile-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("created sessions publish project names and live fallback and explicit titles", async () => {
		...
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-created-title-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
	...
	test("create without title uses authority, and prompt lock failure recovers status for retry", async () => {
		const authority = new FakeAuthority();
		const factory = new LiveFactory();
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-authority-live-"));
		const appserver = createAppserver({
			hostId: host,
			epoch,
			socketPath: nodePath.join(root, "app.sock"),
		...
		const lockApp = createAppserver({
			hostId: host,
			epoch: "lock",
			socketPath: nodePath.join(root, "lock.sock"),
	...
	test("close rejects a stale pending-start revision, then prevents child resurrection", async () => {
		...
		const factory = new LiveFactory();
		const values = [record("s1")];
		const root = await fs.mkdtemp(nodePath.join(os.tmpdir(), "omp-close-lock-"));
		const appserver = createAppserver({
			hostId: host,
			epoch: "close-lock",
			socketPath: nodePath.join(root, "app.sock"),
			discovery: new StaticDiscovery(values),
			sessionAuthority: new FakeAuthority(values),
			childFactory: factory,
			lockCheck: () => gate.lock(),
		});
		await appserver.start();
