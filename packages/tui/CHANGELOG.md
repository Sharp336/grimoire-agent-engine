...
- Fixed WSL inside Windows Terminal treating foreground streaming as safe for eager native scrollback rebuilds. The TUI now preserves manually scrolled Windows Terminal history while assistant/tool output continues streaming, and only performs the deferred cleanup at an explicit checkpoint.
- Fixed Windows Terminal sessions (native Windows and WSL) repainting the live viewport during assistant/tool streaming while the host scroll position is unobservable. Structural inserts above a fixed prompt/footer now preserve manually scrolled readers instead of rewriting the visible viewport on every streaming frame.
