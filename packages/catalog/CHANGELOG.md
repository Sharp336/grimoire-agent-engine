...
- Fixed a Friendli GLM-5.2 model with an explicit `compat.supportsReasoningEffort: false` override collapsing to `reasoningDisableMode: "omit"`, which made the enabled request path emit neither `reasoning_effort` (suppressed by the override) nor `chat_template_kwargs.enable_thinking` (omitted by the default branch) — the user could never turn thinking on. `isFriendliToggleOnly` now uses the override-immune `friendliHasEffortSurface` (computed from spec before `applyCompatOverrides`) so identity-known effort models keep `qwen-template-false` and the toggle survives the override.
- Fixed Friendli toggle-only reasoning models (e.g. GLM-4.5) being impossible to disable: the `"omit"` `reasoningDisableMode` left an explicit `disableReasoning` emitting only `delete params.reasoning` (the default branch) with no `chat_template_kwargs.enable_thinking: false`, so the server kept reasoning on at its default. Toggle-only models now get a single-tier binary thinking config from `mapFriendliThinking` and use `qwen-template-false`, which sends `enable_thinking: false` on both the not-requested default and an explicit disable.
- Fixed Friendli dynamic discovery rejecting a zero API price (`"0"`) as invalid and falling back to the seeded reference cost, so a free or promotional model reporting `"0"` could still show the reference's 0.6/2.2. `toPositiveNumberOrNull` now accepts zero as a valid non-negative price and uses the reference only for missing or invalid values.
