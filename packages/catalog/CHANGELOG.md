...
- Added `requesty` catalog entry and `requestyModelManagerOptions` with a custom model mapper — filters to tool-calling chat models, converts Requesty's per-token pricing to per-million, and correctly gates cache read/write costs on `supports_caching` (non-caching models echo `input_price` into `cached_price`; suppressing it avoids false pricing data)
