...
// the model manager rebuilds via `buildModel` on load. v8 invalidates rows
// predating the distinct `max` thinking tier; v7 invalidates rows predating the
// Antigravity Gemini budget-mode migration (cached specs still carrying
// `thinking.mode: "google-level"` and the old 3.5-flash effort routing); v6
// invalidates rows that may contain the retired unknown-limit sentinels
