...
export function collapseEffortVariants<TSpec extends VariantSpecLike>(
	...
	for (const family of table.families) {
		...
		for (const effortKey in family.routing) {
			...
			const preserveAbsent =
				target !== undefined &&
				(family.preserveAbsentRoutes === true ||
					(effort !== "off" && family.preserveAbsentEffortRoutes === true));
			if (target !== undefined && (targetPresent || preserveAbsent) && !retired?.has(target)) {
		...
		if (defaultWireId === family.id) {
			if (usedAbsentEffortRoute && family.preserveAbsentRoutes !== true) {
