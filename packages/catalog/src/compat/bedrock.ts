import { supportsAdaptiveThinkingDisplay, supportsMidConversationSystemMessages } from "../identity/family";
import type { ModelSpec, ResolvedBedrockCompat } from "../types";
import { applyCompatOverrides } from "./apply";

type BedrockPromptCacheCompat = Pick<
	ResolvedBedrockCompat,
	"promptCacheMode" | "supportsLongPromptCacheRetention" | "promptCacheMinimumTokens" | "promptCacheMaximumCheckpoints"
>;
...
/** Resolve Bedrock Converse cache, system-role, and stream-watchdog compat once per model. */
export function buildBedrockCompat(spec: ModelSpec<"bedrock-converse-stream">): ResolvedBedrockCompat {
	const compat: ResolvedBedrockCompat = {
		...
		supportsMidConversationSystem: supportsMidConversationSystemMessages(spec.id),
		streamIdleTimeoutMs: spec.reasoning
