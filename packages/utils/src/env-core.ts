...
/**
 * Record that OMP itself injected `key` from the project `.env` (applied by
 * `env.ts`'s eager side-effect block). `filterChildShellEnv` uses this to strip
 * project-dotenv values that would otherwise leak into child shells.
 */
export function markProjectEnvLoaded(key: string): void {
	projectEnvNamesLoadedByOmp.add(key);
}
 ...
 *
 * `$env` is a live alias of `Bun.env` — reads reflect whatever `.env` values
 * have been applied so far. The application itself lives in `env.ts`'s eager
 * side-effect block (project → agent → config-root → home `.env`), which runs
 * only when `env.ts` is imported directly. The CLI entry reaches it via
 * `help-extra` (after `setProfile`), and the SDK entry via `agent-session`;
 * both happen before any business code reads `$env`. Standalone tools that
 * import `$env` from the `@oh-my-pi/pi-utils` barrel and need `.env` values
 * MUST import `@oh-my-pi/pi-utils/env` explicitly (the barrel re-exports the
