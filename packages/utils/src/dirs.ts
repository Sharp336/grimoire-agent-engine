...
export function getTestAuthPath(agentDir?: string): string {
	return dirs.agentSubdir(agentDir, "testauth.db", "data");
}

/** Get the memories directory (~/.oh-omp/agent/memories). */
export function getMemoriesDir(agentDir?: string): string {
	return dirs.agentSubdir(agentDir, "memories", "state");
}

/** Get the terminal sessions directory (~/.oh-omp/agent/terminal-sessions). */
