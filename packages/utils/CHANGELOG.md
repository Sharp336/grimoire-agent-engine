...
- Parsed absolute quota-reset moments named in GLM Coding Plan 429 error bodies (`您的限额将在 2026-08-17 11:17:40 重置`, codes 1308–1321; international Z.AI `Your limit will reset at 2026-02-06 05:34:34`) so `extractRetryHint` returns the real remaining wait (+3s grace) instead of falling through to a generic hint. The English wording additionally requires a Zhipu 13xx error code in the body, so a foreign provider's `will reset at` body keeps its own hints. A zone marker in the stamp is honored as-is (any case); documented stamps are bare wall-clock server time per Zhipu support, and both platforms anchor in UTC+8, so bare stamps parse as +08:00. Per the same support advice, a bare stamp stale past a 1-minute plausibility window is reread as UTC; a just-past stamp is retried once the boundary is a full window past (plus grace). Out-of-calendar stamps (`2026-02-30`) are rejected rather than rolled over by `Date.parse`. Stamps whose reread is still past, or beyond 7 days (the weekly quota is the largest window Zhipu documents), yield nothing. To let the session sleep through such multi-hour resets instead of failing fast, raise or disable `retry.maxDelayMs` (`0` = no cap).
