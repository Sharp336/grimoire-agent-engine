...
9. **Publish.** Before calling `gh_open_pr`, re-check the issue thread for a linked/open PR, concrete patch, or maintainer note that a fix is in flight. If an existing fix applies, do NOT open a duplicate PR; reply with one `gh_post_comment` referencing it instead. Otherwise call `gh_push_branch`, then `gh_open_pr`. Both deterministically run `bun run fix` (amending any formatter diff into your HEAD commit) then `bun check` before touching the remote. The same gate runs on every follow-up `gh_push_branch`. The tools also refuse dirty trees and commit-author mismatches.
