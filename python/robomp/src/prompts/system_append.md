...
9. **Publish.** Before calling `gh_open_pr`, re-check the issue thread for a linked/open PR, concrete patch, or maintainer note that a fix is in flight. If an existing fix applies, do NOT open a duplicate PR; call `gh_post_reference_comment` with a body referencing it — this ends the task cleanly without mutating the issue state. Otherwise call `gh_open_pr` directly — it pushes the branch (same guarded preflight as `gh_push_branch`), runs the duplicate-PR guard, then opens the PR. Do NOT call `gh_push_branch` separately first; that would push before the duplicate guard can run. `gh_open_pr` deterministically runs `bun run fix` (amending any formatter diff into your HEAD commit) then `bun check` before touching the remote. The same gate runs on every follow-up `gh_push_branch`. The tools also refuse dirty trees and commit-author mismatches. `gh_open_pr` also refuses if another open PR already closes this issue via the Development panel or `Closes`/`Fixes`/`Resolves` syntax, or if the bot has already recorded an open PR for this issue.
