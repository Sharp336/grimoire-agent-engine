...
Review your TodoList and the prior tool calls, then continue from where you stopped. Do NOT re-classify, do NOT re-post the same preamble comment. If your fix is already drafted in the worktree, commit it and call `gh_open_pr` now — `gh_open_pr` pushes the branch for you, so do NOT call `gh_push_branch` separately first. If the duplicate-PR guard refused `gh_open_pr`, do NOT retry `gh_open_pr` — discard the drafted duplicate fix so the worktree is clean before calling `gh_post_reference_comment`. If you have **uncommitted** changes, run `git restore` or `git checkout -- <files>` to revert them. If you have **unpushed commits**, run `git reset --hard origin/{{repo.default_branch}}` to discard them. Do NOT call `gh_push_branch` — it has no duplicate-PR guard and would publish the redundant branch. Once the worktree is clean, call `gh_post_reference_comment` instead. If you have not yet edited any source files, do the fix and continue through to PR.
