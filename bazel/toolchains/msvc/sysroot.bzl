...
target-specific CRT/SDK libraries for both shipped Windows architectures:

    splat/crt/include             VC toolset headers
    splat/crt/lib/{x86_64,aarch64}
    splat/sdk/include/{ucrt,um,shared,winrt,cppwinrt}
    splat/sdk/lib/{ucrt,um}/{x86_64,aarch64}
...
_BUILD = """\
# Generated by //bazel/toolchains/msvc:sysroot.bzl — x86_64 + aarch64 MSVC CRT
# and Windows SDK splatted by xwin {version} (manifest-version {manifest}).
# Consumed by the @msvc_cc target-specific wrappers via /imsvc and /libpath.
...
filegroup(
    name = "crt_libs",
    srcs = glob(["splat/crt/lib/x86_64/**"]),
)

filegroup(
    name = "crt_libs_arm64",
    srcs = glob(["splat/crt/lib/aarch64/**"]),
)
...
def _xwin_sysroot_impl(rctx):
    ...
    rctx.report_progress("Splatting x86_64 + aarch64 MSVC CRT and Windows SDK via xwin (first fetch ~1 GiB from the Microsoft CDN)")
...
xwin_sysroot_repository = repository_rule(
    ...
    doc = "Target-specific x86_64 + aarch64 MSVC CRT/Windows SDK sysroots from pinned xwin.",
