...
- **Alt+click, or double click, to copy a whole message** — a user message, an assistant reply, or a tool call's real output. It copies the block's _source_, so a collapsed card yields its entire output rather than the preview on screen. Cmd+click is not offered because SGR mouse reporting has no super bit, and shift+click is not, because terminals reserve shift to bypass mouse reporting.
