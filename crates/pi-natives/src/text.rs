...
/// is detected at runtime via a DSR/CPR probe (see packages/tui terminal.ts)
/// and pushed here through `set_hangul_compat_jamo_width_override`.
...
fn apply_hangul_compat_jamo_delta(width: usize, c: char) -> usize {
	...
	// The probe only measures a *visible* jamo, so only the narrow correction
	// (target 1) applies to the filler; a wide terminal renders it at its
	// Unicode width (0), not the probed visible-jamo width. Never widen a
...
fn char_width_corrected(c: char) -> Option<usize> {
	...
	// (Ghostty, most Linux terminals). The width is detected at runtime via a
	// DSR/CPR probe and applied through the override; absent a probe we fall
	// back to the compile-time platform default.
	if is_hangul_compat_jamo(c)
		&& let Some(target) = hangul_compat_jamo_target_width()
	{
		// Zero-width filler (U+3164): only the narrow correction applies — a
		// wide terminal renders it at its Unicode width (0), not the probed
		// visible-jamo width. See apply_hangul_compat_jamo_delta.
		let unicode_width = UnicodeWidthChar::width(c).unwrap_or(0);
		if unicode_width == 0 && target > 1 {
