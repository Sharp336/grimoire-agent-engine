...
fn run_parallel_search(
	...
	let mut results: Vec<FileSearchResult> = entries
		...
		.map_init(
			...
			|searcher, entry| -> Result<Option<FileSearchResult>> {
				ct.heartbeat()?;
...
fn grep_sync(
		...
		&& use_gitignore
	{
		...
		let mut entries =
			collect_files(&search_path, &scan.entries, glob_set.as_ref(), type_filter.as_ref(), &ct)?;
	...
	let (matches, total_matches, files_with_matches, files_searched, limit_reached) =
		run_sequential_search(&entries, &matcher, params, &ct)?;
