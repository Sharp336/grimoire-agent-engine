...
> accepts Zod _or_ ArkType _or_ JSON Schema, and the public `pi.zod` extension API + the Zod-backed
...
- **ArkType** = a _callable function_ with `.toJsonSchema` and `.assert` methods (`isArkSchema`).
...
1. **Never put `.default()` on an optional `?` key.** `z.X.default(v).optional()` in Zod is
   **output-optional** (default applied in code via `?? `) → translate to an **optional key, no
   default**: `"limit?": "number"`. Only `z.X.default(v)` _without_ `.optional()` (output-required)
...
6. **`toolWireSchema()` removes `$schema` automatically.** If you call
   `toJsonSchema()` directly for some other boundary, strip its `$schema`
   metadata there.
...
- Report any `.strict`→`"+"`, `.refine`→`.narrow`, `.catch`→morph conversion,
  and any file intentionally left on Zod with the reason.
