...
Contexts without a local focusable session use the Hub's full-screen transcript viewer instead. The viewer incrementally tails the file-backed transcript and provides an input line only when the selected agent can be messaged. Sending there has the same semantics: revive if parked, steer if running, and prompt if idle.
...
See also [Task Agent Discovery and Selection](./task-agent-discovery.md).
