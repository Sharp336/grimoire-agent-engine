...
The session bootstrap bridge converts custom tools to extension `ToolDefinition`s and forwards calls in the correct argument order. Library consumers compose a `CustomTool` into an `AgentTool` with `composeCustomTool`, a plain `ToolDefinition` with `composeToolDefinition` (or an already-built native tool with `composeAgentTool`); when a runner (`ExtensionRunner`) is supplied, all route through the single `ExtensionToolWrapper` approval/event gate. **Runner-less composition is an exception:** `composeCustomTool(tool, undefined, { getContext })` returns the meta-noticed adapter *without* an `ExtensionToolWrapper`, so declared approval policies and extension events are **not** applied. An embedder using the runner-less path must not assume the tool's `approval` tier is enforced — provide a runner, or enforce approval externally, before executing destructive custom tools.
