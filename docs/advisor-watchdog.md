...
At each primary turn end, `AdvisorRuntime` receives only the new transcript delta since the last advisor update. Deltas are rendered with `formatSessionHistoryMarkdown(..., { includeThinking: settings.advisor.includeThinking, includeToolIntent: true, watchedRoles: true, expandPrimaryContext: true })`. `advisor.includeThinking` defaults to `true`, so the advisor can review assistant reasoning as well as user-visible text, tool calls, and tool results. Set `advisor.includeThinking: false` (or toggle it in settings) to hide thinking/reasoning traces from the advisor. If a provider refuses a request that echoed reasoning, the runtime can still demote thinking for the current model even when the setting is on.
