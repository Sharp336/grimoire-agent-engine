...
The `tui.scrollbackRebuild` setting (default `false`) changes the repair of a committed-prefix divergence. When finalized content replaces a scrolled-off preview, or when a frame collapses into committed rows, a direct terminal session erases native scrollback with ED3. The session replays the current frame, thus the stale form and the final form do not both remain. Multiplexer sessions never take this destructive path and keep the append and repair-below fallback. `PI_TUI_SCROLLBACK_REBUILD=1` sets the low-level `TUI` flag, but `InteractiveMode` then applies the configured `tui.scrollbackRebuild` value. In coding-agent, the setting value overrides this flag. `InteractiveMode` also turns the repair on in a HerdR session when the setting has no explicit value.
