...
- With the `tui.tmuxHistoryRebuild` setting enabled (or `PI_TUI_TMUX_HISTORY_REBUILD=1`) and synchronized output active, a direct tmux width change takes an explicit replacement path instead. Width changes observed under a visible overlay latch a pending replacement that runs on the first uncovered render, even though the overlay frame already committed the new geometry. The renderer first composes the complete current-width frame non-destructively, confirms no visible overlay owns a later fallible render, passes the Ghostty initial-image deferral gate, and resolves every component-provided width-epoch callback. Only after that authoritative state exists does it clear pane history. It then begins DEC 2026 synchronization, clears the grid, waits until `tmux capture-pane` acknowledges a blank grid, clears history once more so the old viewport cannot re-enter scrollback, and emits the already-composed full replay; no component render or width-epoch callback runs after the first destructive clear. The history commands never reset pane modes. If the first clear fails, the renderer keeps the ordinary bounded width-epoch repaint; if the grid acknowledgment or second clear fails, the first already removed old history, the replay still restores the current frame, and later replacement attempts are disabled. The path requires self-reported tmux 3.7+ and is disabled when `STY`, `ZELLIJ`, cmux markers, or direct HerdR identify a nested/competing multiplexer owner. This avoids stale-width and duplicate scrollback without ED3, but intentionally discards pre-OMP history in the direct tmux pane when successful.
