...
新 `forkVersion` 必须是稳定的 `X.Y.Z`，在 SemVer 顺序上高于 npm `latest`，且从未发布过。稳定发布入口不接受 prerelease 或 build metadata。已经发布 `17.2.11` 后，`17.2.11-cn.1` 反而是更低的 prerelease，不能作为下一版本；桥接版本至少需要等待同号的 `17.2.12` native，或按第 1 节走经批准的手动迁移方案。
...
核心原则：保留上游业务逻辑，在明确的 i18n、分叉身份、安装和发行边界重新接入中文能力。禁止对目录批量执行 `git checkout --ours` 或 `--theirs`。
