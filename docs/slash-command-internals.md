...
`/pause` is available only in the interactive TUI. It engages a process-global gate for the main agent, in-process subagents. Each agent parks at its next safe boundary: in-flight calls finish, nothing is aborted, and no new work starts until the gate is released.
