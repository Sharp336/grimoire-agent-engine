...
Choosing a context engine in `/settings` applies immediately. Changes made directly in a configuration file apply when settings reload. `context.lossless.*` changes update an enabled runtime without canceling work already in flight.
...
Choosing **Native** in `/settings` applies immediately. If you edit a configuration file instead, reload settings to stop the active LCM runtime. Disabling LCM stops new claims, drains fenced finalizers, and closes the derived store. Session history needs no conversion because LCM never rewrites it.
