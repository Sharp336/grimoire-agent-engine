...
|| `tools.xdevPromote`            | array   | `[]`    | Tool names kept top-level instead of mounted under `xd://` while `tools.xdev` is on (e.g. `lsp`, `mcp__context7_resolve_library_id`). Promotion only changes presentation — tools still need their normal enablement — and is a no-op when `tools.xdev` is off. |
