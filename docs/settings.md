...
| `compaction.preferProviderNative` | boolean | `false` | Prefer provider-native compaction over `snapcompact` for auto-compaction and a bare `/compact` when the selected compaction candidate supports it. Explicit `/compact snapcompact` remains local. |
