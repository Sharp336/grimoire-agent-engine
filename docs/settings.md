...
`session_metrics` displays the journal's compaction count across all branches and its current JSONL size (for example, ` 3/1.5 MB 󰆓`). It is available for placement in either custom segment list.
