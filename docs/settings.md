...
| ---------------------- | ------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `modelRoles`           | record  | `{}`                        | Map of role name -> model id. Built-in roles: `default`, `smol`, `slow`, `vision`, `plan`, `designer`, `commit`, `tiny`, `task`. The `tiny` role overrides the online model for lightweight background tasks (titles, auto-thinking, unexpected-stop), else `@smol`. Per-role env/flags exist only for `--model`/`--smol`/`--slow`/`--plan`. |
...
- Agent behavior and safety: `ask.*`, `eval.*`, `features.*`, `goal.*`, `loop.*`, `model.loopGuard.*`, `model.toolCallLoopGuard.*`, `recap.*`, `tools.*`, and `vault.*`.
...
- Interface and startup: `display.*`, `statusLine.*`, `startup.*`, `tui.*`, and `ttsr.*`.
- Integrations, storage, and discovery: `async.*`, `bashInterceptor.*`, `commands.*`, `dev.*`, `exa.*`, `gc.*`, `github.*`, `magicKeywords.*`, `mcp.*`, `providers.*`, `searxng.*`, `skills.*`, `task.*`, `todo.*`, and `workspace.*`.
