...
compaction:
  ...
  remoteEnabled: true
  preferProviderNative: false # opt in to native compaction ahead of snapcompact when supported
...
| `compaction.preferProviderNative` | boolean | `false` | Prefer provider-native compaction over `snapcompact` for auto-compaction and a bare `/compact` when the selected compaction candidate is replay-compatible with the active model. Explicit `/compact snapcompact` remains local. |
