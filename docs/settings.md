...
| `task.agentServiceTierOverrides` | record | `{}` | Sparse exact-name overrides for spawned task/eval agents. Values: `inherit`, `none`, `auto`, `default`, `flex`, `scale`, `priority`. An entry overrides `tier.subagent`; concrete values apply only when supported by the resolved model's provider family. |
