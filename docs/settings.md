...
| `autolearn.enabled`                  | boolean | `false`     | Experimental failure-aware procedural memory: recall prior procedures after repeated tool failures and capture new ones as isolated managed skills under `~/.omp/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active), plus `/learn`. |
| `autolearn.captureMode`              | enum    | `recovery`  | `off`, `substantive`, `recovery`, `both`. `substantive` uses the post-stop tool-count trigger; `recovery` captures only a repeated-failure family that later succeeded; `both` prefers recovery and falls back to substantive for that stop.                                          |
| `autolearn.captureDelivery`          | enum    | `automatic` | `automatic` runs one private capture turn at a normal terminal stop; `manual` runs none and leaves capture to an explicit `/learn`.                                                                                                                                                  |
| `autolearn.recallMode`               | enum    | `require`   | `off`, `suggest`, `require`. `suggest` emits up to three compact descriptor cards; `require` soft-requires reading the single best matched `skill://<name>`. Neither injects procedure bodies.                                                                                        |
| `autolearn.failureThreshold`         | number  | `3`         | Same-family tool failures before recall/recovery capture arms. Clamped to `2..10`.                                                                                                                                                                                                   |
| `autolearn.procedureScope`           | enum    | `global`    | `global`, `project-tagged`. Project-tagged procedures gain same-project ranking affinity but stay searchable from every project.                                                                                                                                                     |
| `autolearn.substantiveMinToolCalls`  | number  | `5`         | Minimum tool calls in a turn before substantive capture runs. Clamped to `1..100`.                                                                                                                                                                                                   |
