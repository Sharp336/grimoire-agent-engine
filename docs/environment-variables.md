...
| `BAILIAN_TOKEN_PLAN_API_KEY`    | Bailian (China) Token Plan auth                  | Using `bailian-token-plan-cn` provider                         | Compatible with Qwen Code's Token Plan preset; region-locked to the Beijing endpoint                 |
| `BAILIAN_TOKEN_PLAN_CN_API_KEY` | Bailian (China) Token Plan auth                  | Using `bailian-token-plan-cn` provider                         | Provider-specific name                                                                              |
