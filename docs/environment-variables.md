...
| `OPENCODE_GO_WORKSPACE_ID`      | OpenCode Go `/usage` workspace id                | Showing `opencode-go` quota usage via `/usage`                | Optional when `~/.config/opencode/opencode-quota/opencode-go.json` provides the same value         |
| `OPENCODE_GO_AUTH_COOKIE`       | OpenCode Go `/usage` auth cookie                 | Showing `opencode-go` quota usage via `/usage`                | Optional when `~/.config/opencode/opencode-quota/opencode-go.json` provides the same value         |
