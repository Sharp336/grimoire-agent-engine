...
| `OMP_UPDATE_REGISTRY`        | npm registry `omp update` checks for new versions and installs from (default `https://registry.npmjs.org/`). Must be an `http(s)` URL with no query or fragment; a missing trailing slash is added. `user:pass@` credentials become a Basic `Authorization` header on the version check and are redacted from OMP's output, but are still passed to bun/npm as `--registry=<url>`, so they remain visible in the process list. The `--registry` flag takes precedence. The version check and the install always use the same registry, and `NODE_EXTRA_CA_CERTS` applies to both.                          |
