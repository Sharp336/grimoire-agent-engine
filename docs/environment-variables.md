...
| `CLINE_API_KEY`                 | Cline API usage-credit auth                      | Using `cline-api` provider models or ClinePass fallback        | Cline API gateway at `https://api.cline.bot/api/v1`                                                |
| `CLINEPASS_API_KEY`             | ClinePass subscription auth                      | Using `clinepass` provider models                              | Preferred when separate Cline API and ClinePass keys are configured                                |
