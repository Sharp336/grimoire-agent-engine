...
| `OMP_UPDATE_REGISTRY`        | npm registry `omp update` checks for new versions and installs from (default `https://registry.npmjs.org/`). Must be an `http(s)` URL; a missing trailing slash is added. The `--registry` flag takes precedence. The version check and the bun/npm install always use the same registry, so a mirror that lags cannot produce a `No version matching "X"` install failure.                                                                                                                                                                                                                                |
