...
| `ALIBABA_TOKEN_PLAN_API_KEY`    | Alibaba Token Plan Enterprise auth               | Using `alibaba-token-plan` provider                            | Key format `sk-sp-...`; targets the Singapore (ap-southeast-1) Model Studio Token Plan endpoint     |
