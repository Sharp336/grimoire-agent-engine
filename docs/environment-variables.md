...
| `CLOUDFLARE_AI_GATEWAY_API_KEY` | Cloudflare AI Gateway auth                       | Using `cloudflare-ai-gateway` provider                         | Base URL is the gateway base `https://gateway.ai.cloudflare.com/v1/<account>/<gateway>`. Legacy `/anthropic`-suffixed values are auto-normalized for backward compatibility. Backend is selected by the model ID prefix (`anthropic/`, `openai/`, `workers-ai/`, `google-ai-studio/`). |
