# Session Operations: dump, fresh, clear, fork, resume/continue

This document describes operator-visible behavior for session dumps, conversation reset, lifecycle, fork, and resume operations as currently implemented.
...
- Rotates provider-side session state, and invalidates
...
4. Clear agent and next-turn queues.
...
7. Replace agent messages, and synchronize todos. Close cached provider sessions for a different file, or for a same-file reload whose replay messages changed.
...
12. Finish the bash transition, and notify session-change callbacks when the session id changed.
