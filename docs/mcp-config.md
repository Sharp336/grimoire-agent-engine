...
When the flag is repeated and two files define the same server name, the last one wins — including when the later entry only sets `"enabled": false`, which turns off a server an earlier file defined. A discovered server is also shadowed when an explicit entry points at the same endpoint under a different name, so renaming a server in a generated config does not connect it twice.
