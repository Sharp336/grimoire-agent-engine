...
- Wall clock: `task.maxRuntimeMs` applies to every spawn; default `0` disables it. When set, `task.maxRuntimeSoftPhase` (default `"notice"`) governs the soft phase a subagent that is still producing turns gets before the hard abort: `"notice"` sends a wrap-up steering notice at 25% before the deadline (min 30 s window, max 75% of the cap) then a forced final `yield` at 10% before it (min 15 s, max 50%); `"yield"` sends only that forced final `yield`, no notice; `"off"` disables the soft phase entirely, so `task.maxRuntimeMs` behaves exactly as it did before — hard abort on the timer, nothing else. A child that has stopped producing messages, the provider-hang case, never crosses a soft threshold in any mode and always falls through to the hard abort on the timer.
