...
- Wall clock: `task.maxRuntimeMs` applies to every spawn; default `0` disables it. When set, a subagent that is still producing turns gets the same ladder as the request budget — a wrap-up notice at 25% before the deadline (`task.maxRuntimeNotice`, on by default), a forced final `yield` at 10% before it (floors: 30 s / 15 s, each capped at 75% / 50% of the cap so short caps keep the ordering) — and only a child that has stopped producing messages, the provider-hang case, falls through to the hard abort on the timer.
