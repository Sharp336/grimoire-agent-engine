...
| `isolated` | `boolean` | No | Run in an isolated workspace and return patches. Exists only when `task.isolation.mode` is not `none`, plan mode is disabled, and the calling agent is not itself isolated (unless `task.isolation.allowNested` is enabled, default false). Inside an isolated agent the field is removed from the exposed schema, and an explicit request — on the task tool or the eval `agent()` — is rejected with a clear error when the setting is off; per item in batch shape, top-level in flat shape. Isolated agents are torn down at completion — not revivable. |
