...
- **Provider adapters**
  ...
  - **Gemini** — `packages/coding-agent/src/web/search/providers/gemini.ts`
    ...
    - Model: `providers.webSearchGeminiModel` selects the Gemini grounding model; `GEMINI_SEARCH_MODEL` overrides it. Defaults to `gemini-3.5-flash`. On Cloud Code Assist (OAuth) paths the id is routed through the catalog's `requestModelId` mapping (e.g. `gemini-3.5-flash` → `gemini-3.5-flash-extra-low`); the developer-API path sends it as configured.
