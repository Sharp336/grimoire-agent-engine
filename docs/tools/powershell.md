...
- `details.pid`: PID of the backing `pwsh` host (attach with `Enter-PSHostProcess -Id <pid>`).
...
3. `acquirePsHost()` in `pshost-manager.ts` returns the session's warm host (keyed by `session.getSessionId()`), spawning one on first use and evicting hosts idle beyond `powershell.idleTtlMs`.
...
- Session state
  ...
  - Tool concurrency is `exclusive`; the native serializes pipelines on the shared runspace regardless.
...
## Settings
- `powershell.enabled` (default `true`) — gates tool registration.
