...
- First interactive run shows a one-time data-disclosure consent (`secondOpinion.consented`). Headless runs treat `secondOpinion.enabled = true` as consent and never persist the flag.
...
| `model` | `string` | No | Explicit reviewer selector (`provider/id`, `id`, or substring). Bypasses the configured role and the picker; never persisted. |
...
3. `resolveDefaultReviewer(...)`: configured role (`configured`) → slow model **if cross-family** with the session (`slow`) → any available cross-family model (`fallback`) → slow (`slow`) → first available (`fallback`).
...
- The picker (`context.ui.select` / `context.ui.confirm`) fires when: no stored fingerprint (first run), or the session/slow model **family** changed since the last confirmation. Point releases within a family (e.g. `opus-4.7` → `opus-4.8`) do not re-prompt — the fingerprint stores families, not raw model ids.
...
## Side Effects
- Settings: on interactive confirmation writes `modelRoles.secondopinion`, `secondOpinion.lastPickerFingerprint`, and (first run) `secondOpinion.consented`. Reads those plus model usage order and the active model.
