...
4. CLI execution goes through `git.github.run/json/text()` in `packages/coding-agent/src/utils/git.ts`:
   ...
   - A host named by a full URL (a `pr://<host>/…` read, a PR/issue/run URL argument) is preserved as given, including `github.com`, so `GH_HOST` cannot redirect that request. Cache rows drop an explicit `github.com/` prefix, so both spellings of a github.com repository share one row.
