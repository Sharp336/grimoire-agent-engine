...
- Hindsight mental-model defaults: `hindsight.mentalModelsEnabled = true`, `hindsight.mentalModelAutoSeed = true`, `hindsight.mentalModelMaxTokens = 4096`, `hindsight.mentalModelRefreshIntervalMs = 5 * 60 * 1000`, `hindsight.mentalModelMaxRenderChars = 16_000`. `mentalModelMaxTokens` is the server-side reflect completion budget; `mentalModelMaxRenderChars` limits client-side prompt injection. First-turn loading waits up to `MENTAL_MODEL_FIRST_TURN_DEADLINE_MS = 1500`.
- Hindsight seeds are create-only, except automatic seeding upgrades built-in models at the former 600/800-token caps to `mentalModelMaxTokens`. Other existing server-side models are unchanged.
