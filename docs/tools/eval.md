...
JS filesystem/bridge helpers are asynchronous; Python helpers are synchronous. `read()` delegates non-`local://` schemes to the registered read tool, resolves `local://` through injected roots, and reads regular paths relative to cwd. `write()` accepts regular and `local://` paths but rejects other protocol URLs.
...
- Python use keyword arguments (`schema_mode` outside JS).
...
- Python use retained subprocess kernels speaking framed local IPC. JavaScript uses a worker VM.
