...
Supported languages (v1): TypeScript, TSX, JavaScript, Python, Rust, Go, Java. The extractor emits these domain `kind` strings: `function`, `method`, `constructor`, `class`, `interface`, `trait`, `struct`, `union`, `enum`, `enum_member`, `property`, `field`, `constant`, `variable`, `type_alias`, `namespace`, `module`, `macro`. The TS layer maps each to an LSP `SymbolKind` for the TUI icon; the model-facing text uses the raw domain kind string.
