...
The same two behaviors are also reachable as slash commands: `/steer <message>` injects a message into the running turn (the plain-Enter-while-streaming behavior) and `/followup <message>` queues a message to send after it (the `app.message.followUp` behavior). The command name is authoritative — `/steer` always steers and `/followup` always queues, regardless of which key submitted the line — and when the agent is idle either command shows a notice instead of starting a new turn.
