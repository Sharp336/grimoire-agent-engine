...
| `app.clipboard.pasteTextRaw`  | `Ctrl+Shift+V`, `Alt+Shift+V`          | Paste clipboard text literally (no smart image/path handling) |
...
`app.clipboard.pasteImage` and bracketed paste treat a clipboard or pasted string that looks like a single absolute image file path as an image attachment when the file is readable. To paste that path (or any other text) into the prompt exactly as copied — for example a filesystem path you want the model to see as plain text ([#3590](https://github.com/can1357/oh-my-pi/issues/3590)) — use `app.clipboard.pasteTextRaw` (`Ctrl+Shift+V`, `Alt+Shift+V` by default). Raw paste reads `text/plain` from the clipboard and inserts it with `insertText`; it does not attach images, expand `[Paste #N]` markers, or run the large-paste menu. When the clipboard holds only an image (no text), raw paste reports that there is no text to paste. For path-shaped bracketed pastes and clipboard image paths, the **Path Paste** setting (`paste.pathPaste`: ask / always attach / always keep as text) controls whether you get a per-paste menu or a fixed default.
