...
| `app.tools.expand`           | `Ctrl+O`                                                              | Toggle tool-output expansion; expands or collapses the Council pane instead while a run is live                                                                                      |
...
| `app.message.dequeue`        | `Alt+Up`, `Shift+Up`                                                  | Dequeue a queued message back into the editor; `Shift+Up` first scrolls an expanded, scrollable Council pane                                                                         |
...
While a [Council](./council.md) run is live and its pane is on screen, three chords change meaning:

- `Ctrl+O` (`app.tools.expand`) expands or collapses the Council pane instead of transcript tool output. Both conditions must hold: an active run *and* a visible pane. Everywhere else the global behaviour is unchanged.
- `Shift+Up` and `Shift+Down` scroll the pane, but only when it is **expanded** and its viewport can actually move. When the scroll offset cannot change (a collapsed pane, a pane whose content fits, or an already-pinned top or bottom), the keystroke is not consumed and falls through to `app.message.dequeue`, which shares the `Shift+Up` chord. `PgUp` and `PgDn` are consumed by an expanded, scrollable pane and otherwise fall through too.
- `Esc` cancels the run. Council takes `Esc` first once it has reserved your streaming turn for adjudication, and ahead of draft preservation and the double-`Esc` navigation gesture once your session is idle.

`/hotkeys` adds a Council row while a run is live, so the overloads are visible exactly when they apply.
