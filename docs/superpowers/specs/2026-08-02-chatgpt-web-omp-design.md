...
- Browser-only and full-mode login/lease browser starts use the digest/version-bound native verified Chrome executable, an owner-controlled profile directory handle, and a private inherited `--remote-debugging-pipe` transport. A package-private adapter implements the pinned Playwright v1.62.1 `ConnectOverCDPTransport` contract and calls `chromium.connectOverCDP()` only inside the host process; it never exposes a path, endpoint URL, websocket, raw CDP control plane, or ambient environment across the BrowserHost/provider/RPC boundary. Native launch accepts only closed headed/feature-toggle options, derives the profile path from the held handle, injects exactly one pipe/profile flag, and rejects endpoint, sandbox, duplicate-flag, and unknown-argument attempts before spawn. Executable/profile replacement, orphan survival, PID reuse, and stale-owner races fail closed.
