...
| `claude`    | `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md` | User + project | User file `~/.claude/CLAUDE.md` (or `$CLAUDE_CONFIG_DIR`). Project files are discovered by walking up from the current directory to the repository root (or the filesystem root when there is no repository): at each directory, `CLAUDE.md`, `.claude/CLAUDE.md`, and `CLAUDE.local.md` are loaded, with `CLAUDE.local.md` appended last. Multiple existing files in the same directory are merged into a single depth entry. Empty files are skipped. |
...
Providers marked "(no ancestor walk-up)" only look in the current working directory's config directory. If you need ancestor walk-up behavior, prefer the native `.omp/AGENTS.md` format, a standalone `AGENTS.md` (the `agents-md` provider), or a `CLAUDE.md` / `.claude/CLAUDE.md` (the `claude` provider walks ancestors).
...
- `.gemini/GEMINI.md` and `.github/copilot-instructions.md` are read only from the current working directory's config directory — not from every ancestor. `CLAUDE.md` files do walk ancestors (see the `claude` provider row above).
