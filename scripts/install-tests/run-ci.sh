...
smoke_cli() {
   ...
   # Spawns the bundled native workers to check distribution loading.
...
#    handled separately).
for pkg in utils wire omptype hashline catalog ai mnemopi snapcompact agent tui; do
   (
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$omptype_tgz" "$natives_tgz" "$hashline_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$coding_agent_tgz"
