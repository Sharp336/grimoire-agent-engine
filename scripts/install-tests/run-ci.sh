...
for pkg in utils wire app-wire appserver hashline catalog ai mnemopi snapcompact agent tui stats collab-web; do
   (
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$app_wire_tgz" "$appserver_tgz" "$natives_tgz" "$hashline_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$coding_agent_tgz" "$collab_web_tgz"
